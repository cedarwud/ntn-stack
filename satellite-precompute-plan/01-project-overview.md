...
**核心理念**：使用真實 TLE 歷史數據（本地收集）+ 預計算存儲 + 時間軸播放，解決即時計算性能問題，同時保持數據真實性。

### 📊 數據收集策略
- **手動數據收集**：45天 TLE 數據本地存儲 (data/tle/YYYYMMDD/)
- **雙格式支援**：TLE + JSON 格式同步收集
- **實際日期命名**：基於 TLE epoch 實際日期而非下載日期
- **智能更新檢查**：避免重複下載，支援增量更新
...
```
基於本地收集的 TLE 檔案：
- TLE 格式：45 個檔案 × ~6000 顆衛星 × 3 行 ≈ 40MB
- JSON 格式：45 個檔案 × ~6000 顆衛星 ≈ 120MB
- 預計算結果：45 天 × 24 小時 × 120 時間點(30秒) × 8 顆可見衛星 = 518,400 條記錄
- 總存儲需求：518,400 × 500 bytes ≈ 260MB（Docker 映像檔完全可承受）
```
