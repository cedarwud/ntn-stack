...
整合 NetStack 同步演算法與 SimWorld 模擬，提供智能換手決策
"""
...
from .fine_grained_sync_service import (
...
class HandoverTrigger(Enum):
    ...
    PROACTIVE_HANDOVER = "proactive_handover"  # 預測性換手


class HandoverStrategy(Enum):
    """換手策略類型"""

    REACTIVE = "reactive"  # 反應式換手
    PREDICTIVE = "predictive"  # 預測式換手
...
class HandoverDecisionService:
    ...
    def __init__(
        ...
        # 服務依賴
        self.enhanced_orbit_service = (
            enhanced_orbit_service or EnhancedOrbitPredictionService()
        )
        ...
        self.max_handover_latency_ms = 50.0  # 最大換手延遲
    ...
    async def evaluate_handover_triggers(
        ...
        try:
            # 獲取當前衛星覆蓋資訊
            current_coverage = (
                await self.enhanced_orbit_service.check_ue_satellite_coverage(
                    ue_id=ue_id,
    ...
    def _check_elevation_threshold(
        self, coverage: UECoverageInfo
    ) -> Optional[HandoverTriggerCondition]:
        """檢查仰角閾值"""
        trigger = HandoverTriggerCondition(
    ...
    async def _check_predicted_outage(
        ...
        try:
            ...
            prediction = (
                await self.enhanced_orbit_service.binary_search_handover_prediction(
                    ue_id=ue_id,
        ...
        try:
            ...
            best_satellite = await self.fine_grained_service.calculate_best_satellite(
                ue_position=(
                    ...
                    ue_position.altitude or 0.0,
                ),
                timestamp=current_time.timestamp(),
            )

            # 如果最佳衛星不是當前衛星，且品質顯著更好
            signal_improvement = best_satellite.signal_strength - (
                current_coverage.signal_strength_estimate * 100 - 100
            )
            if (
                best_satellite.satellite_id != current_satellite_id
                and signal_improvement > self.quality_improvement_threshold
            ):
    ...
    async def generate_handover_candidates(
        ...
        try:
            ...
            visible_satellites = (
                self.fine_grained_service._generate_mock_visible_satellites(
                    (
                        ue_position.latitude,
                        ue_position.longitude,
                        ue_position.altitude or 0.0,
            ...
            for sat in visible_satellites:
                ...
                # 包括 BLOCKED 和 MARGINAL，因為這些仍可能是可行的換手目標
                ...
                # 計算信號品質改善
                current_signal = (
                    candidate_coverage.signal_strength_estimate * 100 - 100
                )  # 轉為 dBm
            ...
            self.logger.error("換手候選衛星生成失敗", ue_id=ue_id, error=str(e))
            return []
    ...
    async def _estimate_handover_cost(
        ...
        try:
            ...
            self.logger.warning("換手成本估算失敗", error=str(e))
            total_cost = 50.0  # 預設成本

        return total_cost

    def _estimate_switching_latency(
        self, triggers: List[HandoverTriggerCondition]
    ) -> float:
        """估算換手延遲"""
        base_latency = 30.0  # 基礎換手延遲 (ms)

        # 根據觸發條件調整延遲
        for trigger in triggers:
            if trigger.trigger_type == HandoverTrigger.EMERGENCY_HANDOVER:
                base_latency *= 0.8  # 緊急換手優先處理
            elif trigger.trigger_type == HandoverTrigger.PREDICTED_OUTAGE:
                base_latency *= 0.9  # 預測性換手有準備時間
    ...
    async def make_handover_decision(
        ...
        綜合評估觸發條件和候選衛星，做出最終的換手決策
        """
        start_time = time.time()

        try:
            self.logger.info(
                "開始換手決策評估", ue_id=ue_id, current_satellite=current_satellite_id
            )
            ...
            if not triggered_conditions:
                self.logger.info("無觸發條件，無需換手", ue_id=ue_id)
                return None
            ...
            if not candidates:
                self.logger.warning("無可用換手候選衛星", ue_id=ue_id)
                return None
            ...
            decision_reason = self._generate_decision_reason(
                triggered_conditions, best_candidate
            )
    ...
    async def _determine_handover_timing(
        ...
        try:
            ...
            elif candidate.strategy in [
                HandoverStrategy.EMERGENCY_HANDOVER,
                HandoverStrategy.HARD_HANDOVER,
            ]:
                # 緊急或硬換手：立即執行
                return current_time + timedelta(
                    milliseconds=candidate.switching_latency_ms
                )
            ...
            self.logger.debug("換手時機確定異常，使用預設時機", error=str(e))
            return current_time + timedelta(seconds=10)  # 預設延遲

    def _calculate_success_probability(
        self, candidate: HandoverCandidate, triggers: List[HandoverTriggerCondition]
    ) -> float:
        """計算換手成功概率"""
        ...
        for trigger in triggers:
            if trigger.trigger_type == HandoverTrigger.EMERGENCY_HANDOVER:
                base_probability -= 0.05  # 緊急換手風險較高
            elif trigger.trigger_type == HandoverTrigger.PREDICTED_OUTAGE:
                base_probability += 0.05  # 預測性換手準備充分
    ...
    def _generate_decision_reason(
        self, triggers: List[HandoverTriggerCondition], candidate: HandoverCandidate
    ) -> str:
        """生成決策理由"""
        reasons = []
        ...
        reasons.append(
            f"選擇衛星 {candidate.satellite_name}，優先級評分 {candidate.priority_score:.1f}"
        )
    ...
    def _update_decision_stats(
        self, decision: HandoverDecision, decision_time_seconds: float
    ):
        """更新決策統計"""
    ...
    async def execute_multi_satellite_handover(
        ...
        try:
            ...
            self.logger.error("多衛星換手優化失敗", ue_id=ue_id, error=str(e))
            raise
    ...
    async def get_service_status(self) -> Dict[str, Any]:
        ...
        cache_info = {
            ...
            "trigger_history_size": sum(
                len(history) for history in self._trigger_history.values()
            ),
        }
