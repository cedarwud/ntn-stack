...
async def predict_with_weather_integration(
    request: WeatherPredictionRequest, background_tasks: BackgroundTasks
):
    """
    執行天氣整合的衛星預測
    """
    try:
        ...
        # 執行天氣整合預測
        prediction_result = (
            await weather_prediction_service.predict_with_weather_integration(
                ue_id=request.ue_id,
                ue_position=(
                    request.ue_latitude,
                    request.ue_longitude,
                    request.ue_altitude,
                ),
        ...
        if prediction_result["success"]:
            ...
            raise HTTPException(
                status_code=400, detail=prediction_result.get("error", "預測失敗")
            )
        ...
        analysis = {
            "performance_metrics": {
                "prediction_efficiency": (
                    "high" if stats["total_predictions"] > 10 else "low"
                ),
                "confidence_level": (
                    "high" if stats["average_confidence"] > 0.8 else "medium"
                ),
                "cache_utilization": stats["cache_size"]
                / max(stats["total_predictions"], 1),
            },
            "weather_insights": {
                "dominant_conditions": _analyze_weather_distribution(
                    stats["weather_distribution"]
                ),
                "prediction_reliability": stats["average_confidence"],
                "system_stability": (
                    "stable" if stats["total_predictions"] > 0 else "initializing"
                ),
            },
        }
...
async def update_weather_coefficients(
    ...
    rain_attenuation_factor: Optional[float] = Query(
        default=None, description="雨衰係數"
    ),
    cloud_attenuation_factor: Optional[float] = Query(
        default=None, description="雲層衰減係數"
    ),
    reliability_multiplier: Optional[float] = Query(
        default=None, description="可靠性乘數"
    ),
):
    """
    更新天氣影響係數
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail=f"無效的天氣條件: {condition}")
        ...
        # 更新係數
        weather_prediction_service.update_weather_coefficients(
            weather_condition, coefficients
        )
...
async def analyze_atmospheric_impact(
    ...
    try:
        ...
        return {
            ...
            "summary": {
                "min_total_loss_db": (
                    min(a["total_loss_db"] for a in impact_analysis)
                    if impact_analysis
                    else 0
                ),
                "max_total_loss_db": (
                    max(a["total_loss_db"] for a in impact_analysis)
                    if impact_analysis
                    else 0
                ),
                "optimal_elevation_deg": (
                    min(impact_analysis, key=lambda x: x["total_loss_db"])[
                        "elevation_deg"
                    ]
                    if impact_analysis
                    else 0
...
async def _log_weather_prediction_details(
    ue_id: str, prediction_result: Dict, processing_time_ms: float
):
    """記錄天氣預測詳細信息（背景任務）"""
