...
async def execute_two_point_prediction(
    request: HandoverPredictionRequest, background_tasks: BackgroundTasks
):
    """
    執行二點預測算法
    ...
    try:
        ...
        if prediction_result.handover_needed:
            ...
            handover_time, iterations = (
                await fine_grained_service.binary_search_refinement(
                    str(request.ue_id),
                    ue_position,
        ...
        logger.info(
            f"二點預測完成 - UE: {request.ue_id}, 需要換手: {prediction_result.handover_needed}"
        )
...
async def execute_binary_search_refinement(
    ue_id: int, t_start: float, t_end: float, precision_threshold: float = 0.1
):
    """
...
async def start_continuous_prediction(ue_id: int, background_tasks: BackgroundTasks):
    """
    啟動連續預測模式
    """
    try:
        ...
        # 在背景任務中啟動連續預測
        background_tasks.add_task(
            fine_grained_service.start_continuous_prediction, str(ue_id), ue_position
        )
...
async def get_performance_metrics():
    ...
    try:
        ...
        avg_confidence = (
            total_confidence / cached_records if cached_records > 0 else 0.0
        )
        ...
        return {
            ...
            "handover_rate": (
                round(handover_required_count / cached_records * 100, 2)
                if cached_records > 0
                else 0.0
            ),
