...
class ConstrainedSatelliteAccessService:
    ...
    def _initialize_default_constraints(self) -> List[SatelliteConstraint]:
        ...
        return [
            ...
            SatelliteConstraint(
                ...
                weight=0.5,
                is_hard_constraint=False,
                description="空間分離約束，確保換手候選衛星的多樣性",
            ),
        ]
        ...
        try:
            ...
            if best_candidate:
                ...
                await self._record_selection_history(
                    ue_id, best_candidate, scored_candidates
                )

            logger.info(
                f"為 UE {ue_id} 選擇衛星: {best_candidate.satellite_name if best_candidate else 'None'}"
            )
    ...
    async def _build_satellite_candidates(
        ...
        for sat in available_satellites:
            try:
                ...
                coverage_duration = self._estimate_coverage_duration(
                    elevation, orbital_velocity
                )
                ...
                candidate = SatelliteAccessCandidate(
                    ...
                    constraint_scores={},
                    overall_score=0.0,
                    current_ue_count=self.satellite_loads.get(
                        sat.get("norad_id", ""), 0
                    ),
                    max_ue_capacity=100,
                    load_ratio=0.0,
                ...
                # 計算負載率
                candidate.load_ratio = (
                    candidate.current_ue_count / candidate.max_ue_capacity
                )
    ...
    def _calculate_satellite_geometry(
        self, satellite: Dict, ue_position: Tuple[float, float, float]
    ...
    def _calculate_signal_strength(
        self, distance_km: float, elevation_deg: float
    ) -> float:
        ...
        path_loss_db = (
            20 * math.log10(distance_km) + 20 * math.log10(frequency_ghz) + 32.45
        )
    ...
    def _estimate_coverage_duration(
        self, elevation_deg: float, orbital_velocity_km_s: float
    ) -> float:
        """估計衛星覆蓋持續時間"""
        # 簡化模型：基於仰角和軌道速度估計
        if elevation_deg < 10:
    ...
    def _apply_hard_constraints(
        self, candidates: List[SatelliteAccessCandidate]
    ) -> List[SatelliteAccessCandidate]:
        """應用硬約束過濾候選者"""
        filtered = []
        ...
        for candidate in candidates:
            ...
            for constraint in self.constraints:
                ...
                passes_constraint = self._evaluate_hard_constraint(
    ...
    def _evaluate_hard_constraint(
        self, candidate: SatelliteAccessCandidate, constraint: SatelliteConstraint
    ) -> bool:
        """評估硬約束"""
    ...
    def _calculate_constraint_score(
        ...
        if constraint.constraint_type == SatelliteConstraintType.DISTANCE_MAX:
            # 距離越近越好 (歸一化到 0-1)
            normalized = 1.0 - min(
                candidate.distance_km / constraint.threshold_value, 1.0
            )
            return normalized
    ...
    async def _apply_load_balancing(
        self, candidates: List[SatelliteAccessCandidate], ue_id: str
    ) -> List[SatelliteAccessCandidate]:
        """應用負載平衡優化"""
        # 根據當前負載調整候選者評分
        for candidate in candidates:
    ...
    async def _update_assignment(self, ue_id: str, satellite_id: str):
        ...
        if ue_id in self.ue_assignments:
            ...
            if old_satellite_id in self.satellite_loads:
                self.satellite_loads[old_satellite_id] = max(
                    0, self.satellite_loads[old_satellite_id] - 1
                )

        # 更新新分配
        self.ue_assignments[ue_id] = satellite_id
        self.satellite_loads[satellite_id] = (
            self.satellite_loads.get(satellite_id, 0) + 1
        )
    ...
    async def _record_constraint_violations(
        self, candidates: List[SatelliteAccessCandidate]
    ):
        """記錄約束違反統計"""
        for candidate in candidates:
            for constraint in self.constraints:
                ...
                if not self._evaluate_hard_constraint(candidate, constraint):
                    count = self.constraint_violations.get(
    ...
    def get_performance_statistics(self) -> Dict:
        ...
        avg_score = (
            sum(entry["selected_score"] for entry in self.selection_history)
            / total_selections
