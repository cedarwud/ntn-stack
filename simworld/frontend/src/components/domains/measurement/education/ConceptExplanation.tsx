...
const CONCEPT_LIBRARY: Record<ConceptType, ConceptContent> = {
  measurement_events: {
    title: '測量事件基礎',
    description: '了解 5G NTN 系統中的測量事件機制，包括 A4、D1、D2、T1 等事件的作用和觸發條件。',
    ...
    levels: {
      beginner: {
        explanation: '測量事件是 5G 系統中用於監控網路狀態和用戶設備性能的機制。就像汽車的儀表板會顯示速度、油量等資訊，測量事件會報告信號強度、位置變化、時間同步等狀態。',
        keyPoints: [
          'A4 事件：監控位置變化，確保定位準確',
          'D1 事件：測量距離變化，管理服務區域',
          'D2 事件：追蹤移動參考點，如衛星位置',
        ...
        quiz: [
          {
            ...
            options: ['信號強度', '位置變化', '時間同步', '數據傳輸'],
            correctAnswer: 1,
            explanation: 'A4 事件專門用於監控 UE 的位置變化，當位置偏差超過設定門檻時會觸發位置補償。'
          }
        ]
      },
      intermediate: {
        explanation: '測量事件基於 3GPP TS 38.331 標準實現，通過週期性或事件驅動的方式收集網路和 UE 的測量數據。這些數據用於無線資源管理、移動性管理和網路優化。',
        technicalDetails: [
          '事件觸發基於門檻值比較和遲滯機制',
          '測量報告包含時間戳、測量值和事件標識',
          '支援週期性和非週期性報告模式',
        ...
        quiz: [
          {
            ...
            options: ['提高精度', '防止頻繁觸發', '加快響應', '節省功耗'],
            correctAnswer: 1,
            explanation: '遲滯機制通過設定進入和離開門檻的差值，防止在門檻值附近的小幅波動導致頻繁觸發。'
          }
        ]
      },
      advanced: {
        explanation: '高級測量事件實現涉及複雜的信號處理、統計分析和機器學習算法。現代系統使用自適應門檻、預測性觸發和多維度優化來提升性能。',
        researchTopics: [
          '基於 AI 的自適應門檻調整',
          '多事件聯合優化算法',
          '邊緣計算在測量事件中的應用',
        ...
        quiz: [
          {
            question: '在 NTN 環境中，測量事件面臨的主要挑戰是什麼？',
            options: ['高延遲', '都卜勒效應', '路徑損耗', '以上皆是'],
            correctAnswer: 3,
            explanation: 'NTN 環境中，衛星的高速移動導致都卜勒效應，長距離傳輸造成高延遲和路徑損耗，這些都是測量事件需要考慮的挑戰。'
          }
        ]
      }
    },
  ...
  sib19_platform: {
    title: 'SIB19 統一平台',
    description: '深入了解 SIB19 系統資訊廣播的統一平台架構，以及如何實現資訊統一和應用分化。',
    ...
    levels: {
      beginner: {
        explanation: 'SIB19 就像是衛星網路的「廣播電台」，定期向所有用戶設備發送重要的系統資訊，包括衛星位置、時間基準、服務參數等。統一平台確保所有測量事件都使用相同的基礎資訊。',
        keyPoints: [
          'SIB19 提供衛星星曆和時間資訊',
          '統一平台避免資訊重複和不一致',
          '支援多個測量事件的協調工作',
      ...
      intermediate: {
        explanation: 'SIB19 統一平台基於「資訊統一、應用分化」的設計原則，通過單一數據源和事件特定萃取機制，實現高效的資源共享和專業化處理。',
        technicalDetails: [
          '單例模式確保全局唯一數據管理器',
          '事件特定萃取方法提供客製化數據',
          '觀察者模式實現數據更新通知',
      ...
      advanced: {
        explanation: '高級 SIB19 平台實現包括分散式數據同步、智能緩存策略、故障恢復機制和性能監控系統，支援大規模部署和高可用性要求。',
        researchTopics: [
          '分散式 SIB19 數據同步算法',
          '基於機器學習的數據預測和緩存',
...
export const ConceptExplanation: React.FC<ConceptExplanationProps> = ({
  ...
  const renderProgress = () => {
    ...
    const completedQuestions = Object.values(quizProgress).filter(Boolean).length
    const progressPercentage = totalQuestions > 0 ? (completedQuestions / totalQuestions) * 100 : 100
    ...
    return (
      <div className="space-y-4">
        ...
        {levelData.quiz.map((question, index) => (
          <Card key={index} className={quizProgress[index] ? 'border-green-500' : ''}>
            <CardHeader>
    ...
    <div className={`concept-explanation ${className}`}>
      <Card>
        <CardHeader>
              ...
              {onLevelChange && (
                <div className="flex gap-1">
                  {(['beginner', 'intermediate', 'advanced'] as const).map(level => (
                    <Button
                      key={level}
                      variant={level === learningLevel ? 'default' : 'outline'}
                      size="sm"
          ...
          <p className="text-sm text-muted-foreground">{conceptData.description}</p>
        </CardHeader>
            ...
            <TabsContent value="explanation" className="mt-4">
              <div className="space-y-4">
                <div className="p-4 bg-muted rounded-lg">
                  <p className="leading-relaxed">{levelData.explanation}</p>
                </div>
                ...
                {'keyPoints' in levelData && (
                  <div>
                    ...
                    <ul className="space-y-2">
                      {levelData.keyPoints.map((point, index) => (
                ...
                {'technicalDetails' in levelData && (
                  <div>
                    ...
                    <ul className="space-y-2">
                      {levelData.technicalDetails.map((detail, index) => (
                        <li key={index} className="flex items-start gap-2">
                          <div className="w-1.5 h-1.5 rounded-full bg-primary mt-2 flex-shrink-0" />
            ...
            <TabsContent value="examples" className="mt-4">
              <div className="space-y-4">
                {'examples' in levelData && (
                  <div>
                    <h4 className="font-semibold mb-4">實際應用示例</h4>
                ...
                <div>
                  <h4 className="font-semibold mb-4">實際應用領域</h4>
                  <div className="grid gap-2">
                    {conceptData.practicalApplications.map((application, index) => (
                ...
                <div className="grid gap-2">
                  {conceptData.relatedConcepts.map((relatedConcept, index) => (
                    <Button
                      ...
                      onClick={() => {
                        // 這裡可以觸發導航到相關概念
                        console.log('Navigate to:', relatedConcept)
                      }}
                    >
                      <BookOpen className="h-4 w-4 mr-2" />
                      {CONCEPT_LIBRARY[relatedConcept]?.title || relatedConcept}
                    </Button>
                  ))}
                </div>

                <div className="mt-6">
                  <h4 className="font-semibold mb-2">學習前置條件</h4>
                  <div className="space-y-2">
                    {conceptData.prerequisites.map((prerequisite, index) => (
