...
export const EnhancedA4Viewer: React.FC<A4ViewerProps> = ({
    ...
    // 視圖模式管理
    const { currentMode, toggleMode: _toggleMode } = useViewModeManager('a4')

    // 配置更新處理
    const handleConfigChange = useCallback(
        (newConfig: Partial<A4Config>) => {
                ...
                <div className="space-y-3">
                    ...
                    <div>
                        ...
                        <div className="text-xs text-muted-foreground">
                            X: {currentCompensation.x.toFixed(2)}km, Y:{' '}
                            {currentCompensation.y.toFixed(2)}km, Z:{' '}
                            {currentCompensation.z.toFixed(2)}km
                        </div>
                    </div>
            ...
            <CardContent>
                <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                        ...
                        <span className="font-medium">
                            {config.compensationThreshold}km
                        </span>
                    </div>
                    <div className="flex justify-between">
                        <span className="text-muted-foreground">最大範圍:</span>
                        <span className="font-medium">
                            {config.maxCompensationRange}km
                        </span>
                    </div>
                    <div className="flex justify-between">
                        <span className="text-muted-foreground">UE 位置:</span>
                        <span className="font-medium">
                            {config.ueLatitude.toFixed(3)}°,{' '}
                            {config.ueLongitude.toFixed(3)}°
                        </span>
                    </div>
                    <div className="flex justify-between">
                                        ...
                                        {eventHistory.map((event, index) => (
                                            ...
                                            >
                                                ...
                                                <div className="mt-2 text-sm">
                                                    <div>
                                                        補償量: (
                                                        {event.compensation?.x.toFixed(
                                                            2
                                                        )}
                                                        ,{' '}
                                                        {event.compensation?.y.toFixed(
                                                            2
                                                        )}
                                                        ,{' '}
                                                        {event.compensation?.z.toFixed(
                                                            2
                                                        )}
                                                        ) km
                                                    </div>
