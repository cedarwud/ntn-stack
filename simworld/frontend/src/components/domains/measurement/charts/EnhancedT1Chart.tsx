...
const EnhancedT1Chart: React.FC<EnhancedT1ChartProps> = ({
    ...
    const trajectory = useOrbitTrajectory({
        ...
        durationHours: 2,
        intervalMinutes: 1,
        autoUpdate: autoUpdate && showTimeline,
    })
            ...
            tooltip: {
                ...
                callbacks: {
                    ...
                    afterBody: (context: TooltipItem<'line'>[]) => {
                        if (context.length > 0) {
                            const dataIndex = context[0].dataIndex
                            const point = context[0].dataset.data[
                                dataIndex
                            ] as DataPoint
        ...
        scales: {
            ...
            y: {
                ...
                min: function (context) {
                    ...
                    const timeValues = data.flatMap((d) => [
                        d.measurement_values.elapsed_time,
                        d.measurement_values.remaining_service_time,
                    ])
                    const minVal = Math.min(...timeValues)
                    return Math.max(0, minVal - Math.max(10, minVal * 0.1)) // 保留10%餘量，最少10秒
                },
                max: function (context) {
                    if (data.length === 0) return 3600
                    const timeValues = data.flatMap((d) => [
                        d.measurement_values.elapsed_time,
                        d.measurement_values.remaining_service_time,
                    ])
                    const maxVal = Math.max(...timeValues)
                    return maxVal + Math.max(10, maxVal * 0.1) // 保留10%餘量，最少10秒
                },
            },
            y1: {
                type: 'linear' as const,
    ...
    const fetchRealTimeData = useCallback(async () => {
        ...
        try {
            const response = await netstackFetch(
                '/api/measurement-events/T1/data',
                {
                    method: 'POST',
                    headers: {
    ...
    const generateSimulatedData = useCallback(() => {
        ...
        // 計算觸發條件
        simulatedData.trigger_details.elapsed_time =
            simulatedData.measurement_values.elapsed_time
        simulatedData.trigger_details.remaining_service_time =
            simulatedData.measurement_values.remaining_service_time
        simulatedData.trigger_details.threshold_condition_met =
            simulatedData.measurement_values.elapsed_time > t1Threshold
        simulatedData.trigger_details.duration_condition_met =
            simulatedData.measurement_values.remaining_service_time >=
            requiredDuration
        simulatedData.trigger_details.overall_condition_met =
            simulatedData.trigger_details.threshold_condition_met &&
            simulatedData.trigger_details.duration_condition_met
        simulatedData.trigger_details.gnss_time_offset_ms =
            simulatedData.measurement_values.gnss_time_offset_ms
        simulatedData.trigger_details.sync_accuracy_ms =
            simulatedData.measurement_values.sync_accuracy_ms

        simulatedData.trigger_condition_met =
            simulatedData.trigger_details.overall_condition_met
        simulatedData.trigger_state = simulatedData.trigger_condition_met
            ? 'triggered'
            : 'idle'
    ...
    useEffect(() => {
        ...
        const updateFunction = useRealData
            ? fetchRealTimeData
    ...
    return (
        ...
        >
            ...
            {showTimeline && (
                <TimelineAnimator
                    isActive={autoUpdate && showTimeline}
            ...
            <div className="chart-header">
                ...
                <div className="chart-controls">
                    ...
                    <button
                        ...
                        title={showTimeline ? '隱藏時間軸' : '顯示軌跡動畫'}
                    >
                        {showTimeline ? '🕐' : '🚀'}
                    </button>
                </div>
                ...
                <div className="metrics-display">
                    <div className="metric-item">
                        ...
                        <span className="metric-value">
                            {data[
                                data.length - 1
                            ].measurement_values.elapsed_time.toFixed(1)}
                            s
                        </span>
                    </div>
                    <div className="metric-item">
                        ...
                        <span className="metric-value">
                            {data[
                                data.length - 1
                            ].measurement_values.remaining_service_time.toFixed(
                                1
                            )}
                            s
                        </span>
                    ...
                    <div className="metric-item">
                        ...
                        <span className="metric-value">
                            {data[
                                data.length - 1
                            ].trigger_details.sync_accuracy_ms.toFixed(2)}
                            ms
                        </span>
                    </div>
                    <div className="metric-item">
                        ...
                        <span
                            className={`metric-value ${
                                data[data.length - 1].trigger_condition_met
                                    ? 'triggered'
                                    : 'idle'
                            }`}
                        >
                ...
                <div className="trajectory-info-panel">
                    ...
                    <div className="trajectory-metrics">
                        <div className="trajectory-metric">
                            <span className="trajectory-label">位置:</span>
                        ...
                        <div className="trajectory-metric">
                            <span className="trajectory-label">高度:</span>
                            <span className="trajectory-value">
                                {trajectory.currentPoint.altitude.toFixed(1)} km
                            </span>
                        </div>
                        <div className="trajectory-metric">
                            <span className="trajectory-label">距離:</span>
                            <span className="trajectory-value">
                        ...
                        <div className="trajectory-metric">
                            <span className="trajectory-label">仰角:</span>
                            <span className="trajectory-value">
            ...
            {/* 圖表區域 */}
            <div
                className="chart-area h-96 w-full"
