...
export class DataSmoothingService {
    ...
    static identifyTriggerPatterns(
        ...
        for (let i = 0; i < smoothedSatDist.length; i++) {
            ...
            // 計算觸發強度 (距離閾值越遠，觸發越強)
            const cond1Strength = condition1 ? Math.min(1, (satDist - thresh1) / (thresh1 * 0.1)) : 0
            const cond2Strength = condition2 ? Math.min(1, (thresh2 - groundDist) / (thresh2 * 0.1)) : 0
            ...
            if (isTriggered && !currentPattern) {
                ...
                currentPattern.peakStrength = Math.max(currentPattern.peakStrength, triggerStrength)
            } else if (!isTriggered && currentPattern) {
                // 結束觸發模式
                const duration = currentPattern.endTime - currentPattern.startTime
                currentPattern.handoverLikelihood = 
                    currentPattern.peakStrength > 0.8 && duration > 30 ? 'high' :
                    currentPattern.peakStrength > 0.5 && duration > 15 ? 'medium' : 'low'
...
export const ImprovedD2Chart: React.FC<ImprovedD2ChartProps> = ({
    ...
    const processedData = useMemo(() => {
        if (!data || data.length === 0) return { smoothedData: [], triggerPatterns: [] }
        ...
        // 應用平滑化算法
        const smoothedSatelliteDistances = DataSmoothingService.exponentialSmoothing(
            DataSmoothingService.movingAverage(satelliteDistances, smoothingWindowSize),
            smoothingAlpha
        )
        const smoothedGroundDistances = DataSmoothingService.exponentialSmoothing(
            DataSmoothingService.movingAverage(groundDistances, smoothingWindowSize),
            smoothingAlpha
        )
    ...
    }, [data, smoothingWindowSize, smoothingAlpha, thresh1, thresh2, hysteresis])

    // 觸發模式變化時的回調
    useEffect(() => {
        if (onTriggerPatternDetected && processedData.triggerPatterns.length > 0) {
    ...
    const chartData: ChartData<'line'> = useMemo(() => {
        if (!processedData.smoothedData || processedData.smoothedData.length === 0) {
        ...
        datasets.push({
            label: '衛星距離 (平滑)',
            data: processedData.smoothedData.map(d => d.smoothedSatelliteDistance / 1000),
        ...
        datasets.push({
            label: '地面距離 (平滑)',
            data: processedData.smoothedData.map(d => d.smoothedGroundDistance / 1000),
        ...
        if (showRawData) {
            datasets.push({
                label: '衛星距離 (原始)',
                data: processedData.smoothedData.map(d => d.satelliteDistance / 1000),
            ...
            datasets.push({
                label: '地面距離 (原始)',
                data: processedData.smoothedData.map(d => d.groundDistance / 1000),
    ...
    const yAxisRanges = useMemo(() => {
        ...
        const satDistances = processedData.smoothedData.map(d => d.smoothedSatelliteDistance / 1000)
        const groundDistances = processedData.smoothedData.map(d => d.smoothedGroundDistance / 1000)
    ...
    const chartOptions: ChartOptions<'line'> = useMemo(() => ({
                ...
                callbacks: {
                    title: (context) => {
                        ...
                        return [
                            ...
                            `衛星: ${dataPoint?.satelliteInfo?.name || 'Unknown'}`,
                        ]
                    },
                    label: (context) => {
                        ...
                        return [
                            ...
                            `觸發狀態: ${dataPoint?.triggerConditionMet ? '✅ 已觸發' : '❌ 未觸發'}`,
                        ]
                    },
                    afterBody: (context) => {
                        const dataIndex = context[0].dataIndex
                        ...
                        if (dataPoint?.triggerConditionMet) {
                            return [
                                ...
                                `遲滯值: ${(hysteresis / 1000).toFixed(1)} km`,
                            ]
                        }
                        return []
                    },
                ...
                annotations: {
                    ...
                    ...(currentTime !== undefined && {
                        currentTimeLine: {
                            ...
                            borderWidth: 3,
                            label: {
                                content: `當前: ${currentTime.toFixed(1)}s`,
                                enabled: true,
                                position: 'top' as const,
                    ...
                    ...(highlightTriggerPatterns && 
                        Object.fromEntries(
                            processedData.triggerPatterns.map((pattern, index) => [
                                `triggerPattern_${index}`,
                                {
                                    ...
                                    yMin: yAxisRanges.satelliteRange.min,
                                    yMax: yAxisRanges.satelliteRange.min + 
                                          (yAxisRanges.satelliteRange.max - yAxisRanges.satelliteRange.min) * 0.1,
                                    backgroundColor: pattern.handoverLikelihood === 'high' ? 
                                        `${theme.triggerHigh}40` :
                                        pattern.handoverLikelihood === 'medium' ?
                                        `${theme.triggerMedium}40` :
                                        `${theme.triggerLow}40`,
                                    borderColor: pattern.handoverLikelihood === 'high' ? 
                                        theme.triggerHigh :
                                        pattern.handoverLikelihood === 'medium' ?
                                        theme.triggerMedium :
                                        theme.triggerLow,
        ...
        theme, isDarkTheme, showThresholdLines, highlightTriggerPatterns,
        thresh1, thresh2, hysteresis, yAxisRanges, processedData,
        currentTime, onTimeChange, onDataPointClick
    ])

    // 圖表準備回調
    const handleChartReady = useCallback((chart: ChartJS) => {
            ...
            <div style={{
                ...
                {processedData.triggerPatterns.length > 0 && (
                    <div style={{
                        ...
                        高可能性: {processedData.triggerPatterns.filter(p => p.handoverLikelihood === 'high').length}
                    </div>
                )}
