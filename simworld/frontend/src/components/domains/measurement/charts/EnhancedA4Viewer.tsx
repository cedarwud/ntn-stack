...
const EnhancedA4Viewer: React.FC = () => {
    ...
    const [sib19Status, setSib19Status] = useState<SIB19PositionStatus | null>(null)
    const [a4Statistics, setA4Statistics] = useState<A4Statistics | null>(null)
    const [currentSignalQuality, setCurrentSignalQuality] = useState<{ serving: number, neighbor: number }>({ serving: 0, neighbor: 0 })
    ...
    const fetchSIB19PositionStatus = useCallback(async () => {
        try {
            const response = await netstackFetch('/api/measurement-events/sib19-position-status')
            if (response.ok) {
    ...
    const fetchA4Statistics = useCallback(async () => {
        try {
            const response = await netstackFetch('/api/measurement-events/A4/statistics')
            if (response.ok) {
                const data = await response.json()
    ...
    const handleTriggerEvent = useCallback((eventData: any) => {
        const newRecord: EventTriggerRecord = {
            ...
            compensation_status: Math.abs(eventData.position_compensation_m) > 1000 ? 'high' : 'normal',
    ...
    const loadPresetLocation = useCallback((preset: string) => {
        const presets = {
            ...
            mountain: { latitude: 23.8, longitude: 121.0, altitude: 1500 },
            rural: { latitude: 23.5, longitude: 120.5, altitude: 50 }
        }
    ...
    const loadPresetScenario = useCallback((scenario: string) => {
        const scenarios = {
            'strong-signal': { a4_threshold: -80.0, hysteresis: 3.0, time_to_trigger: 160 },     // 強信號環境
            'normal-signal': { a4_threshold: -90.0, hysteresis: 2.0, time_to_trigger: 160 },    // 正常信號環境
            'weak-signal': { a4_threshold: -100.0, hysteresis: 1.0, time_to_trigger: 320 },     // 弱信號環境
            'handover-sensitive': { a4_threshold: -85.0, hysteresis: 0.5, time_to_trigger: 40 }, // 敏感切換
            'handover-stable': { a4_threshold: -95.0, hysteresis: 5.0, time_to_trigger: 640 },   // 穩定切換
            'indoor': { a4_threshold: -105.0, hysteresis: 1.5, time_to_trigger: 256 }            // 室內環境
        }
    ...
    return (
        <div className={`enhanced-a4-viewer ${isDarkTheme ? 'dark-theme' : 'light-theme'}`}>
            {/* 主標題和控制欄 */}
            <div className="viewer-header">
                <div className="title-section">
                    <h2>📡 增強版 A4 信號強度測量事件監測</h2>
                    <div className="subtitle">
                        基於 3GPP TS 38.331 規範 | SIB19 位置補償 | 鄰居衛星信號監測
                    </div>
                </div>
                ...
                <div className="control-buttons">
                    ...
                    <button 
                        className={`control-btn ${showCompensationPanel ? 'active' : ''}`}
                        onClick={() => setShowCompensationPanel(!showCompensationPanel)}
                        title="位置補償"
                    >
                        🗺️
                    </button>
                    <button 
                        className={`control-btn ${showStatisticsPanel ? 'active' : ''}`}
            ...
            <div className="status-bar">
                ...
                <div className="status-item">
                    ...
                    <span className={`status-value ${currentSignalQuality.serving > -90 ? 'good' : 'poor'}`}>
                        {currentSignalQuality.serving.toFixed(1)} dBm
                    </span>
                </div>
                <div className="status-item">
                    <span className="status-label">鄰居信號:</span>
                    <span className={`status-value ${currentSignalQuality.neighbor > -90 ? 'good' : 'poor'}`}>
                        {currentSignalQuality.neighbor.toFixed(1)} dBm
                    </span>
                </div>
                <div className="status-item">
                    <span className="status-label">觸發次數:</span>
                    <span className="status-value">{triggerHistory.length}</span>
                </div>
            </div>
                        ...
                        <div className="panel-content">
                            ...
                            <div className="config-section">
                                ...
                                <div className="preset-buttons">
                                    ...
                                    <button onClick={() => loadPresetLocation('mountain')}>山區</button>
                                    <button onClick={() => loadPresetLocation('rural')}>鄉村</button>
                                </div>
                            </div>
                            ...
                            <div className="config-section">
                                ...
                                <div className="input-group">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => setA4Params(prev => ({
                                            ...prev,
                                            a4_threshold: parseFloat(e.target.value) || -90
                                        }))}
                                    />
                                    <small>鄰居信號超過服務信號此值時觸發</small>
                                </div>
                                <div className="input-group">
                                    <label>遲滯值 (dB)</label>
                                    <input
                                        ...
                                        onChange={(e) => setA4Params(prev => ({
                                            ...prev,
                                            hysteresis: parseFloat(e.target.value) || 2.0
                                        }))}
                                    />
                                ...
                                <div className="preset-scenarios">
                                    ...
                                    <div className="scenario-buttons">
                                        <button onClick={() => loadPresetScenario('strong-signal')}>
                                            強信號環境
                                        </button>
                                        <button onClick={() => loadPresetScenario('normal-signal')}>
                                            正常信號環境
                                        </button>
                                        <button onClick={() => loadPresetScenario('weak-signal')}>
                                            弱信號環境
                                        </button>
                                        <button onClick={() => loadPresetScenario('handover-sensitive')}>
                                            敏感切換
                                        </button>
                                        <button onClick={() => loadPresetScenario('handover-stable')}>
                                            穩定切換
                                        </button>
                                        <button onClick={() => loadPresetScenario('indoor')}>
                                            室內環境
                                        </button>
                                    </div>
                                </div>
                    ...
                    <div className="side-panel history-panel">
                        ...
                        <div className="panel-content">
                            <div className="history-stats">
                                ...
                                <div className="stat-item">
                                    <span className="stat-label">當前服務信號:</span>
                                    <span className="stat-value">{currentSignalQuality.serving.toFixed(1)} dBm</span>
                                </div>
                                <div className="stat-item">
                                    <span className="stat-label">當前鄰居信號:</span>
                                    <span className="stat-value">{currentSignalQuality.neighbor.toFixed(1)} dBm</span>
                                </div>
                            </div>
                                    ...
                                    triggerHistory.map((record, index) => (
                                        <div key={index} className="history-item">
                                            ...
                                            <div className="history-details">
                                                <div>服務 RSRP: {record.trigger_details?.serving_rsrp?.toFixed(1)} dBm</div>
                                                <div>原始鄰居 RSRP: {record.trigger_details?.original_neighbor_rsrp?.toFixed(1)} dBm</div>
                                                <div>補償後 RSRP: {record.trigger_details?.compensated_neighbor_rsrp?.toFixed(1)} dBm</div>
                                                <div>位置補償: {(record.trigger_details?.position_compensation_m / 1000)?.toFixed(2)} km</div>
                                                <div>信號補償: {record.trigger_details?.signal_compensation_db?.toFixed(1)} dB</div>
                                                <div>鄰居衛星: {record.neighbor_satellite}</div>
                                                <div className="trigger-conditions">
                                                    補償狀態: {record.compensation_status === 'high' ? '⚠️ 高補償' : '✅ 正常'}
                                                    觸發條件: {record.trigger_details?.condition_met ? '✅' : '❌'}
                                                </div>
                                            </div>
                                        </div>
                                    ))
                        ...
                        <div className="panel-content">
                            {sib19Status ? (
                                <div className="compensation-status">
                                    ...
                                    <div className="status-item">
                                        <span className="status-label">位置補償:</span>
                                        <span className={`status-value ${sib19Status.compensation_enabled ? 'enabled' : 'disabled'}`}>
                                            {sib19Status.compensation_enabled ? '✅ 啟用' : '❌ 停用'}
                                        </span>
                                    </div>
                                    <div className="status-item">
                                        <span className="status-label">最後計算:</span>
                                        <span className="status-value">
                                            {new Date(sib19Status.last_calculation_time).toLocaleTimeString()}
                                        </span>
                                    </div>
                                    <div className="status-item">
                                        <span className="status-label">位置精度:</span>
                                        <span className="status-value">{sib19Status.position_accuracy_m.toFixed(0)} m</span>
                                    </div>
                                    <div className="status-item">
                                        <span className="status-label">使用衛星:</span>
                                        <span className="status-value">{sib19Status.satellites_used}</span>
                                    </div>
                                    <div className="status-item">
                                        <span className="status-label">補償範圍:</span>
                                        <span className="status-value">±{sib19Status.compensation_range_km.toFixed(1)} km</span>
                                    </div>
                                    <div className="status-item">
                                        <span className="status-label">信號補償範圍:</span>
                                        <span className="status-value">
                                            {sib19Status.signal_compensation_db_range[0].toFixed(1)} ~ 
                                            {sib19Status.signal_compensation_db_range[1].toFixed(1)} dB
                                        </span>
                                    </div>
                                </div>
                            ) : (
                                <div className="loading-status">載入位置補償狀態中...</div>
                            )}
                        </div>
                    </div>
                )}
                            ...
                            {a4Statistics ? (
                                <div className="statistics-display">
                                    <div className="stats-section">
                                        <h4>📊 測量統計</h4>
                                        <div className="stat-item">
                                            <span className="stat-label">總測量次數:</span>
                                            <span className="stat-value">{a4Statistics.total_measurements}</span>
                                        </div>
                                        <div className="stat-item">
                                            <span className="stat-label">觸發事件:</span>
                                            <span className="stat-value">{a4Statistics.trigger_events}</span>
                                        </div>
                                        <div className="stat-item">
                                            <span className="stat-label">觸發率:</span>
                                            <span className="stat-value">{(a4Statistics.trigger_rate * 100).toFixed(1)}%</span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="stats-section">
                                        <h4>📡 信號統計</h4>
                                        <div className="stat-item">
                                            <span className="stat-label">平均服務 RSRP:</span>
                                            <span className="stat-value">{a4Statistics.avg_serving_rsrp.toFixed(1)} dBm</span>
                                        </div>
                                        <div className="stat-item">
                                            <span className="stat-label">平均鄰居 RSRP:</span>
                                            <span className="stat-value">{a4Statistics.avg_neighbor_rsrp.toFixed(1)} dBm</span>
                                        </div>
                                        <div className="stat-item">
                                            <span className="stat-label">平均位置補償:</span>
                                            <span className="stat-value">{(a4Statistics.avg_position_compensation / 1000).toFixed(2)} km</span>
                                        </div>
                                        <div className="stat-item">
                                            <span className="stat-label">平均信號補償:</span>
                                            <span className="stat-value">{a4Statistics.avg_signal_compensation.toFixed(1)} dB</span>
                                        </div>
                                    </div>
                                    
                                    <div className="stats-section">
                                        <h4>🛰️ 鄰居衛星分布</h4>
                                        {Object.entries(a4Statistics.best_neighbor_distribution).map(([satellite, count]) => (
                                            <div key={satellite} className="stat-item">
                                ...
                                <div className="loading-status">載入統計信息中...</div>
                            )}
                        </div>
                    </div>
                )}
