...
const EnhancedD1Chart: React.FC<EnhancedD1ChartProps> = ({
    ...
    // 數據狀態
    const [realTimeData, setRealTimeData] = useState<RealTimeD1Data | null>(
        null
    )
    const [simulationData, setSimulationData] =
        useState<SimulationD1Data | null>(null)
    const [dataHistory, setDataHistory] = useState<
        Array<{ time: number; data: RealTimeD1Data }>
    >([])
    ...
    const fetchRealTimeData = useCallback(async () => {
        ...
        try {
            const response = await netstackFetch(
                '/api/measurement-events/D1/data',
                {
                    method: 'POST',
    ...
    const fetchSimulationData = useCallback(async () => {
        ...
        try {
            const response = await netstackFetch(
                '/api/measurement-events/D1/simulate',
                {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
    ...
    const chartData = useMemo(() => {
        if (useRealData && dataHistory.length > 0) {
            // 真實數據模式：使用歷史數據點
            const servingSatelliteDistancePoints = dataHistory.map(
                (entry, index) => ({
                    x: index,
                    y: entry.data.measurement_values.ml1_distance || 0,
                })
            )

            const referencePositionDistancePoints = dataHistory.map(
                (entry, index) => ({
                    x: index,
                    y: entry.data.measurement_values.ml2_distance || 0,
                })
            )
            ...
            const servingSatelliteDistancePoints = results.map(
                (result, index) => ({
                    x: index,
                    y:
                        result.measurement_values?.ml1_distance ||
                        result.measurement_values?.serving_satellite_distance ||
                        0,
                })
            )

            const referencePositionDistancePoints = results.map(
                (result, index) => ({
                    x: index,
                    y:
                        result.measurement_values?.ml2_distance ||
                        result.measurement_values
            ...
            return {
                servingSatelliteDistancePoints: defaultData.map(
                    (d) => d.servingSatelliteDistance
                ),
                referencePositionDistancePoints: defaultData.map(
                    (d) => d.referencePositionDistance
                ),
