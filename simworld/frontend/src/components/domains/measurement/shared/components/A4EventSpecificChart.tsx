...
export const A4EventSpecificChart: React.FC<A4EventSpecificChartProps> = ({
  ...
  // A4 數據更新處理
  const handleA4DataUpdate = useCallback((data: A4VisualizationData) => {
    setA4Data(data)
    setIsLoading(false)
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            ...
            <div className="text-center">
              ...
              <div className="text-xs text-muted-foreground">原始 ΔS (km)</div>
            </div>
            
            {/* 有效位置補償 */}
            <div className="text-center">
              <div className="text-2xl font-bold text-green-600">
                {position_compensation.effective_delta_s.toFixed(3)}
              </div>
              <div className="text-xs text-muted-foreground">有效 ΔS (km)</div>
            </div>
            
            {/* 幾何補償 */}
            <div className="text-center">
              <div className="text-2xl font-bold text-purple-600">
                {position_compensation.geometric_compensation_km.toFixed(3)}
              </div>
              <div className="text-xs text-muted-foreground">幾何補償 (km)</div>
            </div>
          </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <ScatterChart>
                ...
                <XAxis 
                  ...
                  dataKey="x" 
                  domain={[-5, 5]}
                  label={{ value: 'X 軸補償 (km)', position: 'insideBottom', offset: -5 }}
                />
                <YAxis 
                  type="number" 
                  dataKey="y" 
                  domain={[-5, 5]}
                  label={{ value: 'Y 軸補償 (km)', angle: -90, position: 'insideLeft' }}
                />
                <RechartsTooltip 
                ...
                <Scatter 
                  data={[{
                    x: position_compensation.delta_s * Math.cos(45 * Math.PI / 180),
                    y: position_compensation.delta_s * Math.sin(45 * Math.PI / 180),
                    size: Math.abs(position_compensation.effective_delta_s) * 100
                  }]}
                  fill="#3b82f6"
                />
              </ScatterChart>
  ...
  const renderSignalStrength = () => {
    ...
    const signalQuality = signal_strength.rsrp_dbm > signal_strength.threshold ? 'good' : 'poor'
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">服務衛星</span>
                <Badge variant="outline">{signal_strength.serving_satellite}</Badge>
              </div>
              <div className="text-lg font-bold">
                {signal_strength.rsrp_dbm.toFixed(1)} dBm
              </div>
              <Progress 
                value={Math.max(0, (signal_strength.rsrp_dbm + 120) / 40 * 100)}
                className="h-2"
              />
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">目標衛星</span>
                <Badge variant="outline">{signal_strength.target_satellite}</Badge>
              </div>
              <div className="text-lg font-bold">
                {(signal_strength.rsrp_dbm - 5).toFixed(1)} dBm
              </div>
              <Progress 
                value={Math.max(0, (signal_strength.rsrp_dbm - 5 + 120) / 40 * 100)}
                className="h-2"
              />
            </div>
          </div>
          ...
          <div className="flex items-center justify-center gap-2 p-4 rounded-lg bg-muted">
            ...
            <span className="font-medium">
              信號品質: {signalQuality === 'good' ? '良好' : '較差'}
            </span>
            <Badge variant={signalQuality === 'good' ? 'default' : 'destructive'}>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={historicalData}>
                  ...
                  <RechartsTooltip 
                    ...
                    formatter={(value) => [`${value} dBm`, 'RSRP']}
                  />
                  <Line 
                    type="monotone" 
                    dataKey="rsrp_dbm" 
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="text-center">
              <div className={`text-2xl font-bold ${
                trigger_conditions.is_triggered ? 'text-red-600' : 'text-green-600'
              }`}>
                {trigger_conditions.is_triggered ? '已觸發' : '未觸發'}
              </div>
              <div className="text-xs text-muted-foreground">觸發狀態</div>
            ...
            <div className="text-center">
              ...
              <div className="text-xs text-muted-foreground">遲滯值</div>
            </div>
            ...
            <div className="text-center">
              ...
              <div className="text-xs text-muted-foreground">觸發時間</div>
            </div>
          </div>
          ...
          <div className="mt-4 space-y-2">
            <div className="flex items-center justify-between text-sm">
              <span>觸發進度</span>
              <span>{trigger_conditions.is_triggered ? '100%' : '0%'}</span>
            </div>
            <Progress 
            ...
            <div className="mt-4 h-24">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={historicalData}>
                  ...
                  <RechartsTooltip 
                    labelFormatter={(value) => new Date(value).toLocaleTimeString()}
                    formatter={(value) => [value ? '已觸發' : '未觸發', '狀態']}
                  />
                  <Area 
                    type="stepAfter" 
        ...
        <CardContent>
          <div className="flex items-center justify-center gap-4 p-6">
            ...
            <div className="text-center">
              ...
              <div className="font-medium">{signal_strength.serving_satellite}</div>
              <div className="text-xs text-muted-foreground">服務衛星</div>
            </div>
            
            {/* 切換箭頭 */}
            <div className="flex flex-col items-center">
              <ArrowRight className="h-8 w-8 text-muted-foreground" />
              <div className="text-xs text-muted-foreground mt-1">切換中</div>
            </div>
            ...
            <div className="text-center">
              ...
              <div className="font-medium">{signal_strength.target_satellite}</div>
              <div className="text-xs text-muted-foreground">目標衛星</div>
            </div>
          </div>
          
          {/* 切換條件 */}
          <div className="mt-4 p-3 bg-muted rounded-lg">
            <div className="text-sm font-medium mb-2">切換條件評估</div>
            <div className="space-y-1 text-xs">
              ...
              <div className="flex justify-between">
                <span>遲滯門檻:</span>
                <span>{a4Data.trigger_conditions.hysteresis} dB</span>
              </div>
              <div className="flex justify-between">
                <span>切換建議:</span>
                <span className={a4Data.trigger_conditions.is_triggered ? 'text-green-600' : 'text-red-600'}>
                  {a4Data.trigger_conditions.is_triggered ? '建議切換' : '保持當前'}
                </span>
              </div>
            </div>
          </div>
  ...
  if (isLoading) {
    return (
      <div className={`a4-event-specific-chart ${className}`} style={{ height }}>
        <div className="flex items-center justify-center h-full">
          <div className="text-muted-foreground">載入 A4 事件數據中...</div>
        </div>
      </div>
    )
  }
