...
const EVENT_PRESETS = {
    ...
    D1: {
        default: {
            ...
            reportInterval: 1000,
            reportOnLeave: true,
            referenceLocation1: { lat: 0.033, lon: 0.5654 },
            referenceLocation2: { lat: 0.0478, lon: 0.5173 },
        },
        indoor: {
            Thresh1: 100,
            Thresh2: 50,
    ...
    D2: {
        default: {
            ...
            reportInterval: 1000,
            reportOnLeave: true,
            movingReferenceLocation: { lat: 0, lon: 0 },
            referenceLocation: { lat: 0.033, lon: 0.5654 },
        },
        leo_satellite: {
            Thresh1: 600000,
            Thresh2: 5000,
    ...
    ({
        ...
        const handleEventSelect = useCallback(
            (eventType: EventType) => {
                ...
                setCurrentParams(
                    EVENT_PRESETS[eventType].default as AllEventParams
                )
                setIsEditing(false)
        ...
        const handleParamChange = useCallback(
            (paramName: string, value: number | boolean) => {
            ...
            (event: React.ChangeEvent<HTMLInputElement>) => {
                ...
                reader.onload = (e) => {
                    try {
                        ...
                        if (
                            config.eventType === selectedEvent &&
                            config.parameters
                        ) {
                ...
                <div className="parameter-inputs">
                    <div className="param-section">
                        ...
                        {primaryParams.map((paramName) => {
                            const key = paramName.includes('-')
                                ? paramName.replace(/[-\s]/g, '')
                                : paramName
                            const value =
                            ...
                            {Object.keys(availablePresets).map((presetName) => (
                                ...
                                >
                                    {presetName === 'default'
                                        ...
                                        : presetName === 'outdoor'
                                        ? '🌍 戶外'
                                        : presetName === 'leo_satellite'
                                        ? '🛰️ LEO'
                                        : presetName === 'geo_satellite'
                                        ? '🌌 GEO'
                                        : presetName === 'short_window'
                                        ? '⚡ 短窗口'
                                        : presetName === 'long_window'
                                        ? '⏳ 長窗口'
                                        : presetName}
                                </button>
                            ))}
                            {selectedPreset === 'custom' && (
                                <div className="custom-indicator">✏️ 自訂</div>
                            )}
                        </div>
                    </div>
                )}
                ...
                <div className="config-preview">
                    ...
                    <div className="config-summary">
                        ...
                        <div className="summary-item">
                            ...
                            <code className="summary-formula">
                                {eventConfig.conditions.enter}
                            </code>
                        </div>
                        <div className="summary-item">
                            <span className="summary-label">離開條件:</span>
                            <code className="summary-formula">
                                {eventConfig.conditions.leave}
                            </code>
                        </div>
                        {isEditing && (
