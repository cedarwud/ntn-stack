...
const CoreNetworkSyncViewer: React.FC<CoreNetworkSyncViewerProps> = ({
  ...
  const getRandomComponent = () => {
    const components = ['Access Network', 'Core Network', 'Satellite Network', 'UAV Network', 'Ground Station'];
    return components[Math.floor(Math.random() * components.length)];
  };

  // 生成隨機事件類型
  const getRandomEventType = () => {
    const types = ['Fine-Grained Sync', 'Binary Search Refinement', 'Emergency Resync', 'Component Sync', 'Signalling-Free Coordination'];
  ...
  useEffect(() => {
    ...
    const updateData = () => {
      ...
      setMetrics(prev => {
        ...
        const successfulOps = prev.successfulSyncs + (newEvent.status === 'success' ? 1 : 0);
        const failedOps = prev.failedSyncs + (newEvent.status === 'failed' ? 1 : 0);
      ...
      setIeeeFeatures(prev => ({
        ...prev,
        binarySearchRefinement: prev.binarySearchRefinement + (Math.random() > 0.7 ? 1 : 0)
      }));
      ...
      setComponents(prev => prev.map(comp => ({
        ...
        jitterMs: Math.max(0.1, comp.jitterMs + (Math.random() - 0.5) * 2)
      })));
    };
  ...
  const getStateColor = (state: string) => {
    switch (state) {
      ...
      case 'partial_sync': return '#f59e0b';
      case 'desynchronized': return '#ef4444';
      case 'initializing': return '#6366f1';
  ...
  return (
    <div className="core-sync-dashboard">
      ...
      <div className="service-control">
        <button 
          className={`service-btn ${isServiceRunning ? 'running' : 'stopped'}`}
          onClick={toggleService}
        >
          {isServiceRunning ? '⏸️ 停止服務' : '▶️ 啟動服務'}
        </button>
        <div className="service-status">
          <span className={`status-indicator ${isServiceRunning ? 'running' : 'stopped'}`}></span>
      ...
      <div className="metrics-grid">
        <div className="metric-card primary">
          ...
          <div className="metric-value">{metrics.overallAccuracyMs.toFixed(1)}</div>
          <div className="metric-unit">ms</div>
          <div className="metric-change">最佳: {metrics.maxAchieverAccuracyMs.toFixed(1)}ms</div>
        </div>
        ...
        <div className="metric-card success">
          ...
          <div className="metric-change">{metrics.successfulSyncs}/{metrics.totalSyncOperations}</div>
        </div>
        ...
        <div className="metric-card info">
          ...
          <div className="metric-value">{metrics.signallingFreeOperations}</div>
          <div className="metric-unit">次</div>
          <div className="metric-change">總共 {metrics.totalSyncOperations} 次</div>
        </div>
        ...
        <div className="metric-card warning">
          ...
          <div className="metric-value">{ieeeFeatures.binarySearchRefinement}</div>
          <div className="metric-unit">優化</div>
          <div className="metric-change">{metrics.binarySearchOptimizations} 次操作</div>
        </div>
      </div>
      ...
      <div className="ieee-features">
        ...
        <div className="features-list">
          <div className={`feature-item ${ieeeFeatures.fineGrainedSync ? 'active' : 'inactive'}`}>
            <span className="feature-status">
              {ieeeFeatures.fineGrainedSync ? '🟢' : '🔴'}
            </span>
            <span className="feature-name">Fine-Grained 同步</span>
          </div>
          <div className={`feature-item ${ieeeFeatures.twoPointPrediction ? 'active' : 'inactive'}`}>
            <span className="feature-status">
              {ieeeFeatures.twoPointPrediction ? '🟢' : '🔴'}
            </span>
            <span className="feature-name">Two-Point 預測</span>
          </div>
          <div className={`feature-item ${ieeeFeatures.signallingFreeCoordination ? 'active' : 'inactive'}`}>
            <span className="feature-status">
          ...
          {components.map(comp => (
            <div key={comp.name} className="component-item">
              ...
              <div className="component-metrics">
                <span className="metric">精度: {comp.accuracyMs.toFixed(1)}ms</span>
                <span className="metric">可用性: {(comp.availability * 100).toFixed(1)}%</span>
                <span className="metric">延遲: {comp.latencyMs.toFixed(1)}ms</span>
              </div>
            </div>
          ))}
        </div>
          ...
          {recentEvents.slice(0, 6).map(event => (
            <div key={event.id} className={`event-item ${event.status}`}>
              ...
              <div className="event-details">
                ...
                <span className="event-accuracy">{event.accuracy.toFixed(1)}ms</span>
