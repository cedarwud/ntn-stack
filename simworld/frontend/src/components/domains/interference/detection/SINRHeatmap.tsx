...
const SINRHeatmap: React.FC<SINRHeatmapProps> = ({ 
    ...
    const [realInterferenceMetrics, setRealInterferenceMetrics] = useState<InterferenceMetrics | null>(null)
    const [realSINRTexture, setRealSINRTexture] = useState<THREE.Texture | null>(null)
    const [dataMode, setDataMode] = useState<'simulation' | 'real' | 'hybrid'>('hybrid')
    ...
    useEffect(() => {
        ...
        const fetchInterferenceMetrics = async () => {
            try {
                const response = await fetch('http://localhost:8888/api/v1/interference/metrics')
                if (response.ok) {
    ...
    useEffect(() => {
        ...
        const fetchRealSINRMap = async () => {
            try {
                const response = await fetch('http://localhost:8888/api/v1/simulations/sinr-map')
                if (response.ok) {
            ...
            {useRealData && (
                <group position={[80, 20, 80]}>
                    <mesh>
                        ...
                        <meshBasicMaterial 
                            color={
                                dataMode === 'real' ? 0x00ff00 : 
                                dataMode === 'hybrid' ? 0xffff00 : 
                                0xff0000
                            }
                            transparent
                            opacity={0.8}
        ...
        <div className="sinr-legend" style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', marginBottom: '8px' }}>
                ...
                <div style={{
                    ...
                    borderRadius: '50%',
                    backgroundColor: 
                        dataMode === 'real' ? '#00ff00' : 
                        dataMode === 'hybrid' ? '#ffff00' : 
                        '#ff0000',
                    marginLeft: 'auto'
                }} />
                <span style={{ fontSize: '10px', marginLeft: '4px' }}>
                    {dataMode === 'real' ? '真實' : 
                     dataMode === 'hybrid' ? '混合' : '模擬'}
                </span>
            </div>
            ...
            {realMetrics && dataMode !== 'simulation' && (
                <div style={{ borderTop: '1px solid #555', paddingTop: '8px' }}>
                    <h5 style={{ margin: '0 0 6px 0', fontSize: '11px' }}>📊 真實干擾指標</h5>
                    <div style={{ fontSize: '10px' }}>
                        <div>檢測數量: {realMetrics.total_detections}</div>
                        <div>準確率: {realMetrics.detection_accuracy.toFixed(1)}%</div>
                        <div>回應時間: {realMetrics.average_response_time_ms.toFixed(1)}ms</div>
                        <div>吞吐量改善: {realMetrics.throughput_improvement_percent.toFixed(1)}%</div>
                        <div>延遲降低: {realMetrics.latency_reduction_ms.toFixed(1)}ms</div>
                        <div>可靠性改善: {realMetrics.reliability_improvement_percent.toFixed(1)}%</div>
                    </div>
                </div>
            )}
        </div>
