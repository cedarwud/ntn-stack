...
import { ViewModeManager, useVisibleParameters, useEducationMode } from '../../hooks/useViewModeManager'
...
interface EnhancedParameterPanelProps {
    ...
    showPresets?: boolean
    presets?: Array<{ name: string; label: string; values: Record<string, any> }>
}
...
const EnhancedParameterPanel: React.FC<EnhancedParameterPanelProps> = ({
    ...
    const visibleParameterDefs = useMemo(() => {
        return parameters.filter(param => visibleParameters.includes(param.name))
    }, [parameters, visibleParameters])

    // 驗證狀態
    const [validationErrors, setValidationErrors] = useState<Record<string, string>>({})
    const [expandedGroups, setExpandedGroups] = useState<Set<string>>(
        new Set(groups.filter(g => g.defaultExpanded !== false).map(g => g.name))
    )
    ...
    // 渲染單個參數控制
    const renderParameter = useCallback((param: ParameterDefinition) => {
        const hasError = validationErrors[param.name]
        const showEducation = educationMode.enabled && 
                            (educationMode.showDescriptions || educationMode.showPhysicalMeaning)
            ...
            <div key={param.name} className="space-y-2">
                ...
                {showEducation && (
                    <div className="text-xs text-gray-600 space-y-1">
                        {educationMode.showDescriptions && param.description && (
                            <p>{param.description}</p>
                        )}
                        {educationMode.showPhysicalMeaning && param.physicalMeaning && (
                            <p className="italic">💡 {param.physicalMeaning}</p>
                        )}
                    </div>
                )}
            </div>
    ...
    // 渲染參數分組
    const renderGroup = useCallback((groupName: string, groupDef?: ParameterGroup) => {
            ...
            <div key={groupName} className="border border-gray-200 rounded-lg">
                ...
                {groupName !== '_default' && (
                    <div 
                        ...
                        `}
                        onClick={isCollapsible ? () => toggleGroup(groupName) : undefined}
                    >
                        <div className="flex items-center space-x-2">
                            ...
                            {groupDef?.description && educationMode.showDescriptions && (
    ...
    }, [parametersByGroup, expandedGroups, educationMode.showDescriptions, toggleGroup, renderParameter])
        ...
        <div className={`space-y-4 ${className}`}>
            ...
            {Object.keys(validationErrors).length > 0 && (
                <div className="p-3 bg-red-50 border border-red-200 rounded-lg">
                    <h4 className="text-sm font-medium text-red-800 mb-2">參數驗證錯誤：</h4>
