...
const HandoverComparisonDashboard: React.FC<HandoverComparisonDashboardProps> = ({
  ...
  const [selectedScenario, setSelectedScenario] = useState<string>('urban_mobility');
  ...
  const generateComparisonData = (scenario: string): ComparisonResult => {
    const baseLatency = scenario === 'highway_mobility' ? 80 : 
                       scenario === 'urban_mobility' ? 120 : 
                       scenario === 'rural_coverage' ? 100 : 140;
    ...
    const accelerated: HandoverMetrics = {
      ...
      prediction_accuracy: Math.min(98, traditional.prediction_accuracy * 1.45),  // 45% 預測精度提升
      ...
      user_satisfaction: Math.min(5.0, traditional.user_satisfaction * 1.35)  // 35% 用戶滿意度提升
    };

    const improvement = {
      latency: ((traditional.latency - accelerated.latency) / traditional.latency) * 100,
      success_rate: ((accelerated.success_rate - traditional.success_rate) / traditional.success_rate) * 100,
      packet_loss: ((traditional.packet_loss - accelerated.packet_loss) / traditional.packet_loss) * 100,
      throughput: ((accelerated.throughput - traditional.throughput) / traditional.throughput) * 100,
      power_consumption: ((traditional.power_consumption - accelerated.power_consumption) / traditional.power_consumption) * 100,
      prediction_accuracy: ((accelerated.prediction_accuracy - traditional.prediction_accuracy) / traditional.prediction_accuracy) * 100
    };
    ...
    const testInterval = setInterval(() => {
      setTestProgress(prev => {
        ...
        if (newProgress >= 100) {
          ...
          setCurrentTest(prev => prev ? { ...prev, status: 'completed' } : null);
        ...
        <>
          ...
          <div className="comparison-overview">
            <div className="method-card traditional">
              <div className="method-header">
                <h3>📶 傳統換手</h3>
                <span className="method-type">基於RSRP/RSRQ測量</span>
              </div>
              <div className="key-metrics">
                <div className="metric">
                  <span className="label">延遲</span>
                  <span className="value">{latestResult.traditional_metrics.latency.toFixed(1)}ms</span>
                </div>
                <div className="metric">
                  <span className="label">成功率</span>
                  <span className="value">{latestResult.traditional_metrics.success_rate.toFixed(1)}%</span>
                </div>
                <div className="metric">
                  <span className="label">封包遺失</span>
                  <span className="value">{latestResult.traditional_metrics.packet_loss.toFixed(1)}%</span>
                </div>
              </div>
            </div>
            ...
            <div className="vs-indicator">
              ...
              <div className="improvement-summary">
                <span className="improvement-text">延遲減少</span>
                <span className="improvement-value">
                  {latestResult.improvement_percentage.latency.toFixed(0)}%
                </span>
              </div>
            </div>
            ...
            <div className="method-card accelerated">
              ...
              <div className="key-metrics">
                <div className="metric">
                  <span className="label">延遲</span>
                  <span className="value">{latestResult.accelerated_metrics.latency.toFixed(1)}ms</span>
                </div>
                <div className="metric">
                  <span className="label">成功率</span>
                  <span className="value">{latestResult.accelerated_metrics.success_rate.toFixed(1)}%</span>
                </div>
                <div className="metric">
                  <span className="label">封包遺失</span>
                  <span className="value">{latestResult.accelerated_metrics.packet_loss.toFixed(1)}%</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="detailed-comparison">
            <div className="comparison-header">
              ...
              <div className="metric-selector">
                ...
                >
                  ...
                  <option value="power_consumption">功耗</option>
            ...
            <div className="metrics-grid">
              {Object.entries(latestResult.improvement_percentage).map(([metric, improvement]) => {
                const traditionalValue = latestResult.traditional_metrics[metric as keyof HandoverMetrics] as number;
                const acceleratedValue = latestResult.accelerated_metrics[metric as keyof HandoverMetrics] as number;
                
                return (
                  <div key={metric} className="metric-comparison-card">
                    <div className="metric-header">
                      <h4>{getMetricDisplayName(metric)}</h4>
                    ...
                    <div className="metric-bars">
                      <div className="metric-bar traditional">
                        <span className="label">傳統</span>
                        <div className="bar-container">
                          <div 
                            className="bar-fill" 
                            style={{width: '100%'}}
                          ></div>
                          <span className="value">{traditionalValue.toFixed(1)}{getMetricUnit(metric)}</span>
                        </div>
                      </div>
                      
                      <div className="metric-bar accelerated">
                        <span className="label">加速</span>
                        <div className="bar-container">
                          <div 
                            className="bar-fill" 
                            style={{
                              width: `${(acceleratedValue / traditionalValue) * 100}%`
                            }}
                          ></div>
                          <span className="value">{acceleratedValue.toFixed(1)}{getMetricUnit(metric)}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div key={index} className="history-item">
                  ...
                  <div className="history-improvements">
                    <span className="improvement-item">
                      延遲: -{result.improvement_percentage.latency.toFixed(0)}%
                    </span>
                    <span className="improvement-item">
                      成功率: +{result.improvement_percentage.success_rate.toFixed(0)}%
                    </span>
                    <span className="improvement-item">
                      吞吐量: +{result.improvement_percentage.throughput.toFixed(0)}%
                    </span>
                  </div>
                </div>
              ))}
