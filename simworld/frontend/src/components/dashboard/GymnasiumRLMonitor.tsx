...
const GymnasiumRLMonitor: React.FC = () => {
  ...
  const fetchRLStatus = useCallback(async () => {
    try {
      ...
      const mockMetrics: RLEngineMetrics = {
        ...
        algorithm: selectedEngine === 'gymnasium' ? 'DQN' : 'Traditional',
  ...
  const switchEngine = async (newEngine: 'gymnasium' | 'legacy') => {
    try {
      const endpoint = newEngine === 'gymnasium' 
    ...
    try {
      if (isTraining) {
        ...
        const response = await fetch('/api/v1/ai-decision/ai-ran/train', {
          ...
          body: JSON.stringify({ training_episodes: 100, save_interval: 50 })
        });
      ...
      <div className="monitor-grid">
        ...
        <div className="control-panel">
          ...
          <div className="engine-selector">
            <button
              className={`engine-btn ${selectedEngine === 'gymnasium' ? 'active' : ''}`}
              onClick={() => switchEngine('gymnasium')}
            >
              🤖 Gymnasium RL
            </button>
            <button
              className={`engine-btn ${selectedEngine === 'legacy' ? 'active' : ''}`}
          ...
          <div className="training-controls">
            <button
              className={`training-btn ${isTraining ? 'training' : ''}`}
        ...
        {rlMetrics && (
          <div className="engine-metrics">
            <h3>{getEngineStatusIcon(rlMetrics.engine_type)} 引擎狀態</h3>
            <div className="metrics-grid">
              <div className="metric-item">
                <span className="metric-label">算法:</span>
                <span className="metric-value">{rlMetrics.algorithm}</span>
              </div>
              <div className="metric-item">
                <span className="metric-label">環境:</span>
                <span className="metric-value">{rlMetrics.environment}</span>
              </div>
              <div className="metric-item">
                <span className="metric-label">模型狀態:</span>
                <span className={`metric-value status-${rlMetrics.model_status}`}>
                  {rlMetrics.model_status}
                </span>
              </div>
              <div className="metric-item">
                <span className="metric-label">完成集數:</span>
                <span className="metric-value">{rlMetrics.episodes_completed}</span>
              </div>
              <div className="metric-item">
                <span className="metric-label">平均獎勵:</span>
                <span className="metric-value">{rlMetrics.average_reward.toFixed(2)}</span>
              </div>
              <div className="metric-item">
                <span className="metric-label">探索率 (ε):</span>
                <span className="metric-value">{rlMetrics.current_epsilon.toFixed(3)}</span>
              </div>
              <div className="metric-item">
                ...
                <div className="progress-bar">
                  <div 
                    ...
                    style={{ width: `${rlMetrics.training_progress}%` }}
                  />
                  <span className="progress-text">{rlMetrics.training_progress.toFixed(1)}%</span>
                </div>
              </div>
              <div className="metric-item">
                <span className="metric-label">預測準確率:</span>
                <span className="metric-value">{(rlMetrics.prediction_accuracy * 100).toFixed(1)}%</span>
              </div>
              <div className="metric-item">
                <span className="metric-label">響應時間:</span>
                <span className="metric-value">{rlMetrics.response_time_ms.toFixed(1)}ms</span>
              </div>
              <div className="metric-item">
                <span className="metric-label">記憶體使用:</span>
                <span className="metric-value">{rlMetrics.memory_usage.toFixed(0)}MB</span>
              </div>
              {rlMetrics.gpu_utilization && (
                <div className="metric-item">
                  <span className="metric-label">GPU 使用率:</span>
                  <span className="metric-value">{rlMetrics.gpu_utilization.toFixed(1)}%</span>
                </div>
              )}
            </div>
          </div>
                ...
                <div key={serviceName} className="service-item">
                  <div className="service-header">
                    ...
                    <span 
                      ...
                      style={{ backgroundColor: getHealthStatusColor(status.health_status) }}
                    />
                  </div>
                  <div className="service-metrics">
                    <div className="service-metric">
                      <span>請求數: {status.request_count}</span>
                    </div>
                    <div className="service-metric">
                      <span>錯誤率: {(status.error_rate * 100).toFixed(2)}%</span>
                    </div>
                    <div className="service-metric">
                      <span>平均響應: {status.avg_response_time.toFixed(1)}ms</span>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="realtime-logs">
          ...
          <div className="log-container">
            ...
            <div className="log-entry info">
              ℹ️ [14:15:25] 引擎切換至 Gymnasium 模式
            </div>
