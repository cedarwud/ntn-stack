...
export const EnhancedRealD2Chart: React.FC<EnhancedRealD2ChartProps> = ({
    ...
    const detectTriggerEvents = useCallback(
        (
            data: Array<{ satelliteDistance: number; groundDistance: number }>,
            t1: number,
            t2: number,
            hys: number
        ) => {
            ...
            data.forEach((point, index) => {
                ...
                if (triggered && !currentEvent) {
                    ...
                    currentEvent.maxIntensity = Math.max(
                        currentEvent.maxIntensity,
                        intensity
                    )
                } else if (!triggered && currentEvent) {
                    // 結束事件
                    ...
                    events.push({
                        ...
                        duration,
                        likelihood:
                            currentEvent.maxIntensity > 0.8
                                ? 'high'
                                : currentEvent.maxIntensity > 0.5
                                ? 'medium'
                                : 'low',
                    })
                    currentEvent = null
        ...
        const processData = async () => {
            ...
            try {
                ...
                const formattedData = rawData.map((point) => ({
                    satelliteDistance:
                        point.satelliteDistance ||
                        point.satellite_distance ||
                        0,
                    groundDistance:
                        point.groundDistance || point.ground_distance || 0,
                ...
                // 應用智能處理
                const result =
                    intelligentDataProcessor.processRealSatelliteData(
                        formattedData,
    ...
    const chartData: ChartData<'line'> = useMemo(() => {
        ...
        const labels = processingResult.processedData.map(
            (_, index) => index * 10
        )
        const datasets = []
        ...
        datasets.push({
            label: '地面距離',
            data: processingResult.originalData.map(
        ...
        if (showProcessedDataState) {
            datasets.push({
                label: '智能衛星距離',
                data: processingResult.processedData.map(
                    (d) => d.satelliteDistance / 1000
                ),
    ...
    const chartOptions: ChartOptions<'line'> = useMemo(
        () => ({
            ...
            plugins: {
                        ...
                        footer: (_context) => {
                            if (processingResult) {
                                ...
                                return [
                                    '--- 處理品質 ---',
                                    `降噪率: ${(
                                        metrics.noiseReductionRate * 100
                                    ).toFixed(1)}%`,
                                    `視覺清晰度: ${(
                                        metrics.visualClarityScore * 100
                                    ).toFixed(1)}%`,
                                    `物理準確度: ${(
                                        metrics.physicalAccuracyScore * 100
                                    ).toFixed(1)}%`,
                                ]
                            }
                            return []
                ...
                annotation:
                    showTriggerIndicators && processingResult
                        ? {
                              annotations: {
                                  // 閾值線
                                  thresh1Line: {
                ...
                >
                    ...
                    >
                        <input
                            ...
                            onChange={(e) =>
                                setShowOriginalDataState(e.target.checked)
                            }
                        />
                        顯示原始衛星距離
                    </label>
                    ...
                    >
                        <input
                            ...
                            onChange={(e) =>
                                setShowProcessedDataState(e.target.checked)
                            }
                        />
                        顯示智能衛星距離
                    </label>
                ...
                {showProcessingMetrics && (
                    ...
                    >
                        ...
                        <div>
                            ...
                            {(
                                processingResult.processingMetrics
                                    .visualClarityScore * 100
                            ).toFixed(1)}
                            %
                        </div>
                        <div>🎯 觸發: {triggerEvents.length}個</div>
                    </div>
                )}
            </div>
