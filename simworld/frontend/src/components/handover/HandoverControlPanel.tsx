...
const HandoverControlPanel: React.FC<HandoverControlPanelProps> = ({
  ...
  const renderSatelliteOption = (satellite: VisibleSatelliteInfo) => {
    ...
    const signalQuality = Math.max(0, Math.min(100, satellite.elevation_deg * 2)); // 簡化的信號品質計算
      ...
      >
        ...
        <div className="satellite-details">
          <div className="detail-item">
            ...
            <span className="detail-value">{satellite.norad_id}</span>
          </div>
          <div className="detail-item">
            <span className="detail-label">仰角:</span>
            <span className="detail-value">{satellite.elevation_deg.toFixed(1)}°</span>
          </div>
          <div className="detail-item">
            <span className="detail-label">距離:</span>
            <span className="detail-value">{satellite.distance_km.toFixed(0)} km</span>
          </div>
        </div>
        ...
        <div className="signal-quality">
          ...
          <span className="quality-text">{signalQuality.toFixed(0)}%</span>
        </div>
      </div>
    );
  };
  ...
  return (
    <div className={`handover-control-panel ${!isEnabled ? 'disabled' : ''}`}>
      ...
      {currentConnection && (
        <div className="current-connection">
          ...
          <div className="connection-info">
            <div className="connection-name">{currentConnection.satelliteName}</div>
            <div className="connection-details">
              ID: {currentConnection.satelliteId} | 
              仰角: {currentConnection.elevation.toFixed(1)}° | 
              信號: {currentConnection.signalStrength.toFixed(1)} dBm
            </div>
          </div>
        </div>
      )}
      ...
      <div className="satellite-selection">
        ...
        {handoverCandidates.length > 0 ? (
          ...
          <div className="no-satellites">
            ...
            <div className="no-satellites-text">暫無可用的換手目標衛星</div>
          </div>
        )}
      </div>
        ...
        <div className="handover-progress">
          <div className="progress-header">
            ...
            <span className="confidence-score">
              信心度: {(handoverState.confidence * 100).toFixed(0)}%
            </span>
          </div>
          <div className="progress-bar">
            <div 
              ...
              style={{ 
                width: handoverState.status === 'complete' ? '100%' : 
                       handoverState.status === 'failed' ? '100%' :
                       handoverState.status === 'handover' ? '60%' : '30%'
              }}
            ></div>
          </div>
        </div>
          ...
          <div className="confirm-dialog">
            ...
            <div className="dialog-content">
              ...
              <div className="dialog-satellite to">
                {availableSatellites.find(s => s.norad_id === selectedSatelliteId)?.name || '未知'}
              </div>
              <p>嗎？</p>
            </div>
            <div className="dialog-buttons">
              <button className="confirm-btn" onClick={handleConfirmHandover}>
                ✅ 確認
              </button>
