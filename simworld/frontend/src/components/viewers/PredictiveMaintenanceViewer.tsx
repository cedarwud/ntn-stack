...
const PredictiveMaintenanceViewer: React.FC<PredictiveMaintenanceViewerProps> = ({
  devices,
  enabled
}) => {
  ...
  const generateDeviceHealthData = useCallback((): DeviceHealth[] => {
    const deviceTypes: DeviceHealth['device_type'][] = ['satellite', 'gnb', 'uav', 'core_network', 'database']
    
    return Array.from({ length: 12 }, (_, index) => ({
      device_id: `device_${index + 1}`,
      device_name: `${deviceTypes[index % deviceTypes.length].toUpperCase()}-${String(index + 1).padStart(3, '0')}`,
      ...
      next_maintenance_date: new Date(Date.now() + Math.random() * 90 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      ...
      critical_parameters: [
        {
          ...
          severity: ['normal', 'warning', 'critical'][Math.floor(Math.random() * 3)] as any
        },
        {
          ...
          threshold_value: 80,
          trend: ['improving', 'stable', 'degrading'][Math.floor(Math.random() * 3)] as any,
          severity: ['normal', 'warning'][Math.floor(Math.random() * 2)] as any
        },
        {
          parameter_name: '溫度',
      ...
      maintenance_history: Array.from({ length: 3 }, (_, i) => ({
  ...
  const generatePredictionModels = useCallback((): PredictionModel[] => {
    const deviceTypes = ['satellite', 'gnb', 'uav', 'core_network', 'database']
    ...
    return deviceTypes.map((type, index) => ({
      ...
      last_training_date: new Date(Date.now() - Math.random() * 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      training_data_count: Math.floor(Math.random() * 5000 + 10000), // 10k-15k
      feature_importance: [
        { feature_name: 'CPU 使用率歷史', importance_score: Math.random() * 0.3 + 0.2 },
        { feature_name: '記憶體使用模式', importance_score: Math.random() * 0.25 + 0.15 },
        { feature_name: '網路延遲變化', importance_score: Math.random() * 0.2 + 0.1 },
        { feature_name: '錯誤日誌頻率', importance_score: Math.random() * 0.15 + 0.1 },
        { feature_name: '溫度趨勢', importance_score: Math.random() * 0.1 + 0.05 }
      ].sort((a, b) => b.importance_score - a.importance_score)
    }))
  }, [])

  // 模擬生成維護排程數據
  const generateMaintenanceSchedule = useCallback((): MaintenanceSchedule[] => {
    const maintenanceTypes: MaintenanceSchedule['maintenance_type'][] = ['preventive', 'predictive', 'corrective']
    const priorities: MaintenanceSchedule['priority'][] = ['critical', 'high', 'medium', 'low']
    ...
    return Array.from({ length: 8 }, (_, index) => ({
      ...
      scheduled_date: new Date(Date.now() + Math.random() * 60 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      ...
      replacement_parts: [
        {
          ...
          availability: ['in_stock', 'order_required', 'unavailable'][Math.floor(Math.random() * 3)] as any
        },
        {
          part_name: '網路卡',
          quantity: 1,
          cost: Math.floor(Math.random() * 300 + 100),
          availability: ['in_stock', 'order_required'][Math.floor(Math.random() * 2)] as any
        }
      ]
    }))
  }, [])
  ...
  }, [enabled, generateDeviceHealthData, generatePredictionModels, generateMaintenanceSchedule])
    ...
    datasets: [
      {
        ...
        backgroundColor: deviceHealthData.map(d => 
          d.current_health_score >= 80 ? '#4CAF50' :
          d.current_health_score >= 60 ? '#FF9800' : '#F44336'
        ),
        borderColor: deviceHealthData.map(d => 
          d.current_health_score >= 80 ? '#388E3C' :
          d.current_health_score >= 60 ? '#F57C00' : '#D32F2F'
        ),
        borderWidth: 1
      }
    ]
  ...
  const failureProbabilityChartData = {
    ...
    datasets: [
      {
        label: '故障概率 (%)',
        data: deviceHealthData.map(d => d.predicted_failure_probability * 100),
  ...
  const getHealthStatus = (score: number): { text: string; color: string } => {
          ...
          <div className="section device-list">
            ...
            <div className="device-cards">
              {deviceHealthData.map((device) => {
                const healthStatus = getHealthStatus(device.current_health_score)
                return (
                  <div 
                  ...
                  >
                    ...
                    <div className="device-metrics">
                      <div className="metric">
                        <span className="label">健康分數:</span>
                        <span className="value">{device.current_health_score.toFixed(1)}</span>
                      </div>
                      <div className="metric">
                        <span className="label">故障概率:</span>
                        <span className="value">{(device.predicted_failure_probability * 100).toFixed(1)}%</span>
                      </div>
                      <div className="metric">
                        <span className="label">剩餘壽命:</span>
                        <span className="value">{device.remaining_useful_life} 天</span>
                      </div>
                      <div className="metric">
                        <span className="label">下次維護:</span>
                        <span className="value">{device.next_maintenance_date}</span>
                      </div>
                    </div>
                  </div>
                )
                ...
                <tbody>
                  {maintenanceSchedule.map((schedule) => (
                    <tr key={schedule.schedule_id}>
                      ...
                      <td>
                        <span className={`maintenance-type ${schedule.maintenance_type}`}>
                          {schedule.maintenance_type === 'preventive' ? '預防性' :
                           schedule.maintenance_type === 'predictive' ? '預測性' : '矯正性'}
                        </span>
                      </td>
                      <td>{schedule.scheduled_date}</td>
                      <td>{schedule.estimated_duration} 小時</td>
                      <td>
                        <span 
                          className="priority-badge"
                          style={{ backgroundColor: getPriorityColor(schedule.priority) }}
                        >
                          {schedule.priority === 'critical' ? '緊急' :
                           schedule.priority === 'high' ? '高' :
                           schedule.priority === 'medium' ? '中' : '低'}
                        </span>
                      </td>
                      <td>{schedule.assigned_team}</td>
                      <td>${schedule.cost_estimate.toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
