...
class SatelliteCacheManager {
  ...
  async getSatellitePositions(): Promise<{
    ...
    return this.withCache(cacheKey, async () => {
      try {
        // 獲取 Starlink 衛星數據
        const starlinkResponse = await netstackFetch('/api/v1/satellite-ops/visible_satellites?constellation=starlink&count=10&global_view=true')
        const starlinkData = await starlinkResponse.json()
        
        // 獲取 Kuiper 衛星數據（如果沒有 Kuiper 數據，使用預設值）
        let kuiperData = { satellites: [] }
        try {
          const kuiperResponse = await netstackFetch('/api/v1/satellite-ops/visible_satellites?constellation=kuiper&count=10&global_view=true')
