...
describe('🎯 主要頁面組件', () => {
  ...
  describe('MeasurementEventsPage', () => {
    it('應該正確渲染測量事件頁面', async () => {
      ...
      expect(screen.getByTestId('measurement-events-page')).toBeInTheDocument()
          ...
          <div data-testid="modal-overlay" className="modal-overlay">
            <div data-testid="modal-content" className="modal-content">
              ...
              <main data-testid="modal-body">
                <div data-testid="event-details">事件詳情內容</div>
              </main>
            </div>
          </div>
        )
      ...
      const MockEventA4Viewer = ({ data = [] }) => (
        <div data-testid="event-a4-viewer" className="chart-container">
          ...
          <div data-testid="chart-canvas" style={{ height: '400px' }}>
            {data.length > 0 ? (
              <div data-testid="chart-data">圖表數據 ({data.length} 個點)</div>
            ) : (
...
describe('📊 儀表板組件', () => {
  ...
  describe('ChartAnalysisDashboard', () => {
    it('應該正確渲染圖表分析儀表板', async () => {
      const MockDashboard = () => (
        <div data-testid="chart-analysis-dashboard">
          <header data-testid="dashboard-header">
            <h1>圖表分析儀表板</h1>
            <div data-testid="dashboard-controls">
              <button data-testid="refresh-all">全部刷新</button>
          ...
          <main data-testid="dashboard-content">
            <div data-testid="chart-grid" className="chart-grid">
              <div data-testid="chart-a4" className="chart-item">A4 圖表</div>
              <div data-testid="chart-d1" className="chart-item">D1 圖表</div>
              <div data-testid="chart-d2" className="chart-item">D2 圖表</div>
              <div data-testid="chart-t1" className="chart-item">T1 圖表</div>
            </div>
          </main>
        </div>
      )
      ...
      expect(screen.getByTestId('chart-analysis-dashboard')).toBeInTheDocument()
      expect(screen.getByText('圖表分析儀表板')).toBeInTheDocument()
      expect(screen.getByTestId('dashboard-controls')).toBeInTheDocument()
      expect(screen.getByTestId('chart-grid')).toBeInTheDocument()
      
      // 檢查所有圖表項目
      expect(screen.getByTestId('chart-a4')).toBeInTheDocument()
      expect(screen.getByTestId('chart-d1')).toBeInTheDocument()
      expect(screen.getByTestId('chart-d2')).toBeInTheDocument()
      expect(screen.getByTestId('chart-t1')).toBeInTheDocument()
...
describe('🎮 交互功能測試', () => {
  ...
  describe('AnimationController', () => {
    it('應該正確控制動畫播放', async () => {
      ...
      // 初始狀態檢查
      expect(screen.getByTestId('animation-status')).toHaveTextContent('已暫停')
      expect(screen.getByTestId('play-pause-button')).toHaveTextContent('播放')
      
      // 測試播放功能
      await user.click(screen.getByTestId('play-pause-button'))
      expect(screen.getByTestId('animation-status')).toHaveTextContent('播放中')
      expect(screen.getByTestId('play-pause-button')).toHaveTextContent('暫停')
      
      // 測試重置功能
      await user.click(screen.getByTestId('reset-button'))
      expect(screen.getByTestId('animation-status')).toHaveTextContent('已暫停')
      expect(screen.getByTestId('animation-progress')).toHaveTextContent('0/100')
...
describe('🛡️ 錯誤處理測試', () => {
  ...
  it('應該正確處理載入狀態', () => {
    const LoadingComponent = ({ loading = false, data = null, error = null }) => (
    ...
    // 測試載入狀態
    const { rerender } = renderWithProviders(<LoadingComponent loading={true} />)
    expect(screen.getByTestId('loading-spinner')).toBeInTheDocument()
    
    // 測試錯誤狀態
    rerender(<LoadingComponent loading={false} error="網路錯誤" />)
    expect(screen.getByTestId('error-message')).toHaveTextContent('錯誤: 網路錯誤')
...
describe('♿ 輔助功能測試', () => {
  ...
  it('應該提供正確的 ARIA 標籤', () => {
    const AccessibleComponent = () => (
      <div data-testid="accessible-component">
        ...
        <nav aria-labelledby="main-title" data-testid="main-navigation">
          <ul>
            <li>
              <a href="/dashboard" aria-current="page">儀表板</a>
            </li>
            <li>
    ...
    expect(screen.getByRole('button', { name: '刷新所有圖表' })).toBeInTheDocument()
