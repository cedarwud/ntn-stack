...
export const DecisionControlCenterSimple: React.FC<
    DecisionControlCenterSimpleProps
> = ({ className = '' }) => {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            setApiResponseTime((prev) =>
                Math.max(1, prev + (Math.random() - 0.5) * 0.5)
            )
        }, 3000)
