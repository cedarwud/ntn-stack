...
export const CandidateSelectionPanel: React.FC<
    CandidateSelectionPanelProps
> = ({ candidates: propCandidates, onCandidateUpdate }) => {
    ...
    const refreshCandidates = () => {
        const updatedCandidates = candidates.map((candidate) => ({
            ...
            score: Math.max(
                0.1,
                Math.min(1, candidate.score + (Math.random() - 0.5) * 0.1)
            ),
            signalStrength: Math.max(
                0.1,
                Math.min(
                    1,
                    candidate.signalStrength + (Math.random() - 0.5) * 0.1
                )
            ),
            load: Math.max(
                0,
                Math.min(1, candidate.load + (Math.random() - 0.5) * 0.1)
            ),
            lastUpdate: Date.now(),
        }))
    ...
    return (
        <div className="candidate-selection-panel space-y-4">
                ...
                <CardContent className="p-4">
                    <div className="grid grid-cols-2 gap-4 text-sm">
                        ...
                        <div>
                            ...
                            <div className="text-2xl font-bold">
                                {statistics.avgScore.toFixed(2)}
                            </div>
                        </div>
                        <div>
                            <div className="text-orange-600">最佳候選</div>
                            <div className="text-sm font-semibold">
                                {statistics.bestCandidate.name}
                            </div>
                        </div>
                    </div>
                </CardContent>
            ...
            <div className="flex items-center justify-between">
                ...
                <Button variant="outline" size="sm" onClick={refreshCandidates}>
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="mb-3">
                                            <div className="flex items-center justify-between mb-1">
                                                ...
                                                <span
                                                    className={`text-sm font-bold ${scoreGrade.color}`}
                                                >
                                                    {scoreGrade.grade} (
                                                    {(
                                                        candidate.score * 100
                                        ...
                                        <div className="grid grid-cols-2 gap-2 text-xs">
                                            ...
                                            <div className="flex items-center gap-1">
                                                ...
                                                <span>
                                                    仰角: {candidate.elevation}°
                                                </span>
                                            </div>
                                            <div className="flex items-center gap-1">
                                                <MapPin className="w-3 h-3 text-orange-500" />
                                                <span>
                                                    距離: {candidate.distance}km
                                                </span>
                                            </div>
                                        </div>
                            ...
                            return (
                                <div className="space-y-2">
                                    ...
                                    <div className="text-sm text-gray-600">
                                        信號強度:{' '}
                                        {(
                                            selected.signalStrength * 100
                                        ).toFixed(0)}
                                        % | 負載:{' '}
                                        {(selected.load * 100).toFixed(0)}% |
