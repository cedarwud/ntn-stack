...
class TestPhase1Integration:
    ...
    @pytest.mark.asyncio
    async def test_f1_tle_loading(self, test_config, setup_logging):
        """測試F1: TLE載入"""
        logger = setup_logging
        logger.info("🧪 測試F1: TLE載入...")
        
        pipeline = Phase1Pipeline(test_config)
        
        try:
            satellite_data, orbital_positions = await pipeline._execute_f1_tle_loading()
    ...
    @pytest.mark.asyncio
    async def test_f2_filtering(self, test_config, setup_logging):
        """測試F2: 衛星篩選"""
        logger = setup_logging
        ...
        # 先執行Stage 1獲取數據
        satellite_data, _ = await pipeline._execute_f1_tle_loading()
        
        try:
            filtered_candidates = await pipeline._execute_f2_satellite_filtering(satellite_data)
...
if __name__ == "__main__":
    async def run_integration_tests():
        ...
        try:
            # 運行主要測試
            logger.info("📡 測試F1: TLE載入...")
            await test_suite.test_f1_tle_loading(config, logger)
            
            logger.info("🔍 測試F2: 衛星篩選...")  
            await test_suite.test_f2_filtering(config, logger)
