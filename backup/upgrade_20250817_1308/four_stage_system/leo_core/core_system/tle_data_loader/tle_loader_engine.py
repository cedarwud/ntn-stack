...
class EnhancedTLELoaderEngine:
    ...
    def _get_latest_local_tle_files(self) -> Dict[str, str]:
        """動態查找最新的本地TLE數據文件"""
        # 🔧 修復：使用容器內正確的TLE數據路徑
        tle_base_path = Path("/app/tle_data")  # 容器內的正確路徑
        latest_files = {}
    ...
    async def initialize(self):
        ...
        if ENHANCED_MODULES_AVAILABLE:
            try:
                ...
                self.logger.info(f"   - 增強軌道計算器: 精度目標 <{self.f1_targets['max_calculation_accuracy_m']}m")
                self.logger.info(f"   - 數據驗證器: 目標成功率 ≥{self.f1_targets['min_success_rate']*100:.0f}%")
    ...
    async def load_full_satellite_data(self) -> Dict[str, List[TLEData]]:
        ...
        try:
            # 🔥 強制本地載入：直接指定本地TLE文件路徑，繞過所有fallback機制
            self.logger.info("🚀 使用強制本地載入模式，確保使用全量真實衛星數據")
            
            force_local_sources = {
                'starlink': '/app/tle_data/starlink/tle/starlink_20250816.tle',
                'oneweb': '/app/tle_data/oneweb/tle/oneweb_20250816.tle'
            }
            
            # 並行載入多個星座的本地TLE數據
            loading_tasks = []
            for constellation, local_path in force_local_sources.items():
                self.logger.info(f"🎯 強制載入 {constellation}: {local_path}")
            ...
            # 處理載入結果
            for constellation, result in zip(force_local_sources.keys(), constellation_results):
    ...
    def _parse_tle_content(self, content: str, constellation: str) -> List[TLEData]:
        ...
        # 🔥 強制移除樣本限制：確保載入全量衛星數據
        sample_limit = None  # 強制設為None，載入所有衛星
        self.logger.info(f"🚀 已移除樣本限制，將載入 {constellation} 的全量衛星數據")
