...
class LEOCorePipeline:
    ...
    async def execute_complete_pipeline(self):
        ...
        try:
            ...
            # F1: TLE Loader - 載入全量衛星並計算軌道位置
            f1_start = datetime.now(timezone.utc)
            self.logger.info("🛰️ F1: TLE Loader 開始...")
            
            satellite_data, orbital_positions = await self._execute_f1_tle_loading()
            
            f1_duration = (datetime.now(timezone.utc) - f1_start).total_seconds()
            self.pipeline_stats['stage_durations']['f1_tle_loading'] = f1_duration
            self.pipeline_stats['stages_completed'] += 1
            
            self.logger.info(f"✅ F1完成 ({f1_duration:.1f}秒)")
            
            # F2: Satellite Filter - 篩選候選並使用全量軌道位置
            f2_start = datetime.now(timezone.utc)
            self.logger.info("🔍 F2: Satellite Filter 開始...")
            
            filtered_candidates, candidate_orbital_positions = await self._execute_f2_satellite_filtering(satellite_data, orbital_positions)
            
            f2_duration = (datetime.now(timezone.utc) - f2_start).total_seconds()
            self.pipeline_stats['stage_durations']['f2_satellite_filtering'] = f2_duration
            self.pipeline_stats['stages_completed'] += 1
            
            self.logger.info(f"✅ F2完成 ({f2_duration:.1f}秒)")
            
            # F3: Signal Analyzer - A4/A5/D2事件分析
            f3_start = datetime.now(timezone.utc)
            self.logger.info("📊 F3: Signal Analyzer 開始...")
            
            handover_events = await self._execute_f3_signal_analysis(filtered_candidates, candidate_orbital_positions)
            
            f3_duration = (datetime.now(timezone.utc) - f3_start).total_seconds()
            self.pipeline_stats['stage_durations']['f3_signal_analysis'] = f3_duration
            self.pipeline_stats['stages_completed'] += 1
            
            # 將handover_events存儲到pipeline_stats中供最終報告使用
            self.pipeline_stats['handover_events'] = handover_events
            
            self.logger.info(f"✅ F3完成 ({f3_duration:.1f}秒)")
            
            # A1: Dynamic Pool Planner - 模擬退火最佳化
            a1_start = datetime.now(timezone.utc)
            self.logger.info("🔥 A1: Dynamic Pool Planner 開始...")
            
            optimal_pools = await self._execute_a1_pool_optimization(filtered_candidates, candidate_orbital_positions)
            
            a1_duration = (datetime.now(timezone.utc) - a1_start).total_seconds()
            self.pipeline_stats['stage_durations']['a1_pool_optimization'] = a1_duration
            self.pipeline_stats['stages_completed'] += 1
            
            self.logger.info(f"✅ A1完成 ({a1_duration:.1f}秒)")
    ...
    async def _execute_f1_tle_loading(self):
        """執行F1: TLE數據載入"""
    ...
    async def _execute_f2_satellite_filtering(self, satellite_data, orbital_positions):
        """執行Stage 2: 衛星篩選"""
    ...
    async def _execute_f3_signal_analysis(self, filtered_candidates, orbital_positions):
        """執行Stage 3: 信號分析和事件檢測"""
    ...
    async def _execute_a1_pool_optimization(self, filtered_candidates, orbital_positions):
        """執行Stage 4: 動態池最佳化"""
...
def create_default_config():
    ...
    return {
        ...
        'satellite_filter': {
            ...
            'filtering_params': {
                # 🎯 回復原始合理門檻 - 真正問題不在門檻，而在無軌道數據的回退邏輯
                'min_score_threshold': 50.0,        # 回復原始值
                'max_candidates_per_constellation': 100, # 回復原始值
                'min_elevation_threshold': 5.0,     # 回復原始值  
                'min_visible_time_threshold': 10.0   # 回復原始值
            }
        },
        'event_processor': {
            ...
            'event_thresholds': {
                'a4_threshold_db': 3.0,
                'a5_threshold_db': -6.0,
                'd2_threshold_seconds': 2.0
            }
        },
        'optimizer': {
            'algorithm': 'simulated_annealing',
            'target_pools': {
                'starlink': {'min': 10, 'max': 100},  # 回復合理範圍
                'oneweb': {'min': 3, 'max': 50}
            },
            'annealing_params': {
