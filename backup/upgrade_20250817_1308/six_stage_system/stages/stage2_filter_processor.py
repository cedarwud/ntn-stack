...
class Stage2FilterProcessor:
    ...
    def execute_intelligent_filtering(self, stage1_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # F2階段採用動態篩選 - 不設固定數量限制，依據篩選條件動態決定
        # 移除硬編碼數量，讓智能篩選系統根據實際條件動態篩選
        selection_config = None  # 使用動態篩選模式
        
        try:
            # 執行F2階段專用篩選流程（不包含信號品質和事件分析）
    ...
    def save_stage2_output(self, filtered_data: Dict[str, Any]) -> str:
        ...
        filtered_data['metadata'].update({
            'f2_completion': 'intelligent_filtering_complete',
            'f2_timestamp': datetime.now(timezone.utc).isoformat(),
            'ready_for_f3': True,
            'file_generation': 'clean_regeneration'  # 標記為重新生成
        })
        
        # 💾 生成新的F2階段輸出檔案
        logger.info(f"💾 生成新的F2階段輸出檔案: {output_file}")
        with open(output_file, 'w', encoding='utf-8') as f:
            json.dump(filtered_data, f, indent=2, ensure_ascii=False)
            
        # 檢查新檔案大小
        new_file_size = output_file.stat().st_size
        logger.info(f"✅ F2階段數據已保存: {output_file}")
    ...
    def process_f2_filtering(self, f1_file: Optional[str] = None, f1_data: Optional[Dict[str, Any]] = None, 
                      save_output: bool = True) -> Dict[str, Any]:
        """執行完整的F2階段處理流程"""
        logger.info("🚀 開始F2階段：智能衛星篩選")
        
        # 1. 載入F1階段數據（優先使用內存數據）
        if f1_data is not None:
            logger.info("📥 使用提供的F1階段內存數據")
            # 驗證內存數據格式
            if 'constellations' not in f1_data:
                raise ValueError("F1階段數據缺少 constellations 欄位")
            total_satellites = 0
            for constellation_name, constellation_data in f1_data['constellations'].items():
                satellites = constellation_data.get('orbit_data', {}).get('satellites', {})
                total_satellites += len(satellites)
                logger.info(f"  {constellation_name}: {len(satellites)} 顆衛星")
            logger.info(f"✅ F1階段內存數據驗證完成: 總計 {total_satellites} 顆衛星")
        else:
            f1_data = self.load_stage1_output(f1_file)
        
        # 2. 執行智能篩選
        filtered_data = self.execute_intelligent_filtering(f1_data)
        
        # 3. 可選的輸出策略
        output_file = None
        if save_output:
            output_file = self.save_stage2_output(filtered_data)
            logger.info(f"💾 F2階段數據已保存到: {output_file}")
        else:
            logger.info("🚀 F2階段使用內存傳遞模式，未保存檔案")
        
        logger.info("✅ F2階段處理完成")
        # 獲取篩選結果統計
    ...
    def process_stage2(self, stage1_file: Optional[str] = None, stage1_data: Optional[Dict[str, Any]] = None, 
                      save_output: bool = True) -> Dict[str, Any]:
        """向後兼容的階段二處理方法 - 重定向到F2處理方法"""
        logger.warning("⚠️ process_stage2 已棄用，請使用 process_f2_filtering")
        return self.process_f2_filtering(stage1_file, stage1_data, save_output)
