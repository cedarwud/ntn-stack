...
class UnifiedIntelligentFilter:
    ...
    def process_complete_filtering(self, 
        ...
        logger.info(f"✅ 地理篩選完成: {f2_total}/{f1_total} 顆衛星保留 "
                   f"(減少 {geo_stats['overall_reduction']['reduction_rate_percent']:.1f}%)")
        
        # === F3：換手適用性評分 ===
        logger.info("📊 執行F3: 換手適用性評分")
        ...
        f3_total = sum(len(sats) for sats in scored_data.values())
        logger.info(f"✅ 換手評分完成: {f3_total} 顆衛星已評分")
        
        # === A1.1：信號品質評估 (新增) ===
        logger.info("📡 執行A1.1: 信號品質評估")
        signal_enhanced_data = self._enhance_with_signal_quality(scored_data)
        
        # === A1.2：3GPP 事件分析 (新增) ===
        logger.info("🎯 執行A1.2: 3GPP 事件分析")
    ...
    def process_f2_filtering_only(self, 
                                    sgp4_data: Dict[str, Any], 
                                    selection_config: Optional[Dict[str, int]] = None) -> Dict[str, Any]:
        """
        執行F2階段專用的篩選流程（不包含信號品質和事件分析）
        ...
        # === F2.3：換手適用性評分 ===
        logger.info("📊 執行F2.3: 換手適用性評分")
        scored_data = self.handover_scorer.apply_handover_scoring(geo_filtered)
        
        scoring_stats = self.handover_scorer.get_scoring_statistics(scored_data)
        f2_scored_total = sum(len(sats) for sats in scored_data.values())
        logger.info(f"✅ 換手評分完成: {f2_scored_total} 顆衛星已評分")
        
        # === 動態衛星選擇（F2階段結束）===
        if selection_config is None:
            logger.info("🎯 執行動態篩選模式 - 保留所有通過篩選的衛星")
            selected_satellites = scored_data  # F2階段結束，不包含信號品質
        else:
            logger.info("🏆 執行固定數量選擇模式")
            selected_satellites = self.handover_scorer.select_top_satellites(scored_data, selection_config)
        
        final_total = sum(len(sats) for sats in selected_satellites.values())
        logger.info(f"✅ F2階段篩選完成: {final_total} 顆衛星通過篩選")
        ...
        logger.info(f"🎉 F2階段篩選完成: {total_input} → {final_total} 顆衛星 "
                   f"(篩選率: {(1 - final_total/total_input)*100:.1f}%)")
        ...
        result = {
            "metadata": {
                **sgp4_data.get("metadata", {}),
                ...
                "processing_pipeline": [
                    "f1_sgp4_orbit_calculation",
                    "f2.1_constellation_separation", 
                    "f2.2_geographic_filtering",
                    "f2.3_handover_scoring",
                    "f2.4_signal_quality_assessment", 
                    "f2.5_3gpp_event_analysis",
                    "f2.6_top_satellite_selection"
                ],
    ...
    def _build_f2_output(self, 
                           sgp4_data: Dict[str, Any],
                           selected_satellites: Dict[str, List[Dict]],
                           processing_stats: Dict[str, Any]) -> Dict[str, Any]:
        """構建F2階段專用輸出數據格式（只包含篩選後的衛星，大幅減少檔案大小）"""
        
        result = {
            "metadata": {
                **sgp4_data.get("metadata", {}),
                "f2_filtering_completion": "f2_filtering_only",
                "f2_filtering_version": "4.0.0-f1_f2_f3_a1_architecture", 
                "processing_pipeline": [
                    "f1_sgp4_orbit_calculation",
                    "f2.1_constellation_separation",
                    "f2.2_geographic_filtering", 
                ...
                "unified_filtering_results": {
                    ...
                    "starlink_selected": len(selected_satellites.get("starlink", [])),
                    "oneweb_selected": len(selected_satellites.get("oneweb", [])),
                    "processing_quality": "f2_filtering_complete"
                },
                "processing_statistics": processing_stats,
                "ready_for_f3": True  # 標記可以進入F3階段
            },
            "constellations": {}
        }
        ...
        for constellation_name, selected_sats in selected_satellites.items():
            if constellation_name in original_constellations:
                ...
                constellation_data = {
                    ...
                    "selection_quality": "f2_intelligent_filtered",
                    "filtering_stages": [
