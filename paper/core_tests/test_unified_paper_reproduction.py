...
class UnifiedPaperReproductionTest:
    ...
    def add_test_result(self, test_name: str, success: bool, details: str = "", metrics: Dict = None):
        """統一的測試結果記錄"""
    ...
    async def import_paper_modules(self) -> bool:
        ...
        try:
            ...
            from services.algorithm_integration_bridge import AlgorithmIntegrationBridge, BridgeConfiguration, IntegrationMode
            
            # Algorithm 2 模組
            from services.fast_access_prediction_service import FastSatellitePrediction, AccessStrategy, GeographicalBlock, UEAccessInfo
    ...
    async def test_algorithm_1_comprehensive(self) -> bool:
        ...
        try:
            ...
            algo = SynchronizedAlgorithm(
                delta_t=5.0,  # 論文標準週期
                binary_search_precision=0.1  # 100ms 精度
            )
            ...
            try:
                ...
                print(f"✅ 二分搜尋: {search_duration:.1f}ms, 結果: {handover_time is not None}")
                
                # 記錄性能指標
                self.performance_metrics["algorithm_1_binary_search_ms"] = search_duration
            ...
            # 統計 Algorithm 1 結果
            algo1_tests = [binary_search_success, ue_update_success, periodic_success, parallel_success, status_success]
            ...
            self.add_test_result(
                ...
                {"binary_search_ms": self.performance_metrics.get("algorithm_1_binary_search_ms", 0)}
            )
    ...
    async def test_algorithm_2_comprehensive(self) -> bool:
        ...
        try:
            from services.fast_access_prediction_service import FastSatellitePrediction, AccessStrategy, GeographicalBlock
            ...
            # 3. UE 註冊和策略管理測試
            test_ues = [
                ("ue_unified_flexible", AccessStrategy.FLEXIBLE, {"lat": 25.0, "lon": 121.0, "alt": 100.0}),
                ("ue_unified_consistent", AccessStrategy.CONSISTENT, {"lat": 35.0, "lon": 139.0, "alt": 150.0}),
                ("ue_unified_test", AccessStrategy.FLEXIBLE, {"lat": 40.0, "lon": -74.0, "alt": 50.0})
            ]
            ...
            if test_ues:
                ...
                strategy_update_success = await service.update_ue_strategy(test_ue_id, new_strategy)
                updated_strategy = await service.get_access_strategy(test_ue_id)
                
                strategy_management_success = strategy_update_success and updated_strategy == new_strategy
                print(f"✅ 策略管理: {original_strategy.value} → {updated_strategy.value}")
            else:
            ...
            prediction_success = len(satellite_positions) >= len(real_satellites) * 0.8  # 80% 成功率
            print(f"✅ 衛星位置預測: {len(satellite_positions)}/{len(real_satellites)} ({prediction_duration:.1f}ms)")
            
            # 記錄性能指標
            self.performance_metrics["satellite_prediction_ms"] = prediction_duration
            self.performance_metrics["satellite_prediction_count"] = len(satellite_positions)
            
            # 6. 衛星到區塊分配測試
            satellite_blocks = await service.assign_satellites_to_blocks(satellite_positions)
            assignment_success = isinstance(satellite_blocks, dict) and len(satellite_blocks) == len(service.blocks)
            ...
            complete_prediction_success = (
                ...
                len(prediction_result) == len(test_ues[:2])
            )
            ...
            # 統計 Algorithm 2 結果
            algo2_tests = [
                blocks_success, registration_success, strategy_management_success,
                prediction_success, assignment_success, best_satellite_success,
                complete_prediction_success, status_success
            ]
    ...
    async def test_integration_bridge(self) -> bool:
        ...
        try:
            from services.algorithm_integration_bridge import (
                AlgorithmIntegrationBridge, BridgeConfiguration, IntegrationMode
            )
            ...
            print(f"✅ 模式切換: {'成功' if switch_success else '失敗'}")
            ...
            self.add_test_result(
                "整合橋接服務",
                bridge_success,
                f"初始化:{init_success}, 切換:{switch_success}, 狀態:{status_success}"
            )
    ...
    async def generate_unified_report(self, passed_tests: int, total_tests: int):
        ...
        print(f"\n🎓 論文復現狀態:")
        paper_features = {
            "Algorithm 1 實現": any(r["name"] == "Algorithm 1 綜合測試" and r["success"] for r in self.test_results),
            "Algorithm 2 實現": any(r["name"] == "Algorithm 2 綜合測試" and r["success"] for r in self.test_results),
            "二分搜尋算法": "algorithm_1_binary_search_ms" in self.performance_metrics,
            "地理區塊劃分": self.performance_metrics.get("geographical_blocks_count", 0) == 648,
            "衛星位置預測": self.performance_metrics.get("satellite_prediction_count", 0) > 0,
            "整合橋接功能": any(r["name"] == "整合橋接服務" and r["success"] for r in self.test_results)
        }
