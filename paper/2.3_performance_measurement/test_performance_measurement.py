...
class PerformanceMeasurementTester:
    ...
    async def test_handover_measurement_service(self) -> bool:
        ...
        try:
            ...
            if len(recorded_events) == len(test_schemes):
                tests_passed += 1
                print(f"  ✅ 事件記錄完整性: {len(recorded_events)}/{len(test_schemes)}")
            else:
                print(f"  ❌ 事件記錄不完整: {len(recorded_events)}/{len(test_schemes)}")
            
            # 測試 2: 事件數據結構正確性
            valid_events = 0
            for event in recorded_events:
                if (hasattr(event, 'event_id') and 
                    hasattr(event, 'scheme') and
                    hasattr(event, 'latency_ms')):
                    valid_events += 1
            ...
            if all(10 <= lat <= 300 for lat in latencies):
                tests_passed += 1
                print(f"  ✅ 延遲值合理: 範圍 {min(latencies):.1f}-{max(latencies):.1f}ms")
            else:
                print(f"  ❌ 延遲值異常")
    ...
    async def test_four_scheme_comparison(self) -> bool:
        ...
        try:
            ...
            for scheme, params in schemes_data.items():
                # 為每種方案生成 60 個測試事件
                # 預先決定成功/失敗的分佈
                target_success_rate = 0.99 if scheme == self.HandoverScheme.PROPOSED else 0.95
                ...
                for i in range(60):
                    ...
                    result = self.HandoverResult.SUCCESS if i < num_successes else self.HandoverResult.FAILURE
            ...
            if proposed_latency < min(avg_latencies[s] for s in avg_latencies if s != self.HandoverScheme.PROPOSED):
                tests_passed += 1
            ...
            if 5 <= min(all_latencies) and max(all_latencies) <= 300:
                tests_passed += 1
                print(f"  ✅ 延遲範圍合理: {min(all_latencies):.1f}-{max(all_latencies):.1f}ms")
            else:
                print(f"  ❌ 延遲範圍異常")
            ...
            for scheme in schemes_data.keys():
                ...
                successes = sum(1 for e in scheme_events if e.result == self.HandoverResult.SUCCESS)
            ...
            if all(rate >= 0.90 for rate in success_rates.values()):
                tests_passed += 1
                print(f"  ✅ 成功率統計合理: 範圍 {min(success_rates.values()):.1%}-{max(success_rates.values()):.1%}")
            else:
    ...
    async def test_cdf_generation(self) -> bool:
        ...
        try:
            ...
            # 測試 2: 報告數據結構
            if (report and hasattr(report, 'total_events') and 
                hasattr(report, 'scheme_statistics')):
                tests_passed += 1
                print(f"  ✅ 報告數據結構正確")
            else:
                print(f"  ❌ 報告數據結構錯誤")
    ...
    def generate_test_report(self):
        ...
        if self.performance_metrics:
            ...
            if "handover_measurement" in self.performance_metrics:
                ...
                print(f"     - 延遲範圍: {hm_metrics['latency_range'][0]:.1f}-{hm_metrics['latency_range'][1]:.1f}ms")
        ...
        feature_completion = {
            "HandoverMeasurement 服務": any(name == "HandoverMeasurement服務" and result for name, result in self.test_results),
            "四種方案對比測試": any(name == "四種方案對比" and result for name, result in self.test_results),
            "CDF 曲線生成": any(name == "CDF曲線生成" and result for name, result in self.test_results),
            "論文標準數據匯出": any(name == "數據匯出" and result for name, result in self.test_results)
        }
