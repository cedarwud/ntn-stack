...
class MultiSchemeSupportTester:
    ...
    async def setup_test_environment(self) -> bool:
        ...
        try:
            ...
            self.handover_measurement = (
                self.measurement_service.measurement
            )  # 直接引用底層測量對象
    ...
    async def test_scheme_initialization(self) -> bool:
        ...
        try:
            ...
            if len(measurement_schemes) == len(expected_schemes):
                tests_passed += 1
                print(
                    f"  ✅ 測量服務方案支援: {len(measurement_schemes)}/{len(expected_schemes)}"
                )
            else:
                print(
                    f"  ❌ 測量服務方案支援不完整: {len(measurement_schemes)}/{len(expected_schemes)}"
                )

            # 測試 4: 方案特性驗證
            scheme_characteristics = {
                self.HandoverScheme.NTN_BASELINE: {
                    "expected_latency_range": (200, 300)
                },
    ...
    async def test_scheme_differentiation(self) -> bool:
        ...
        try:
            ...
            # 測試 2: 方案分佈均勻性
            scheme_counts = {
                scheme: len(events) for scheme, events in scheme_events.items()
            }
            ...
            other_latencies = [
                lat
                for scheme, lat in scheme_latencies.items()
            ...
            for scheme, config in schemes_config.items():
                ...
                if abs(avg_latency - target_latency) <= acceptable_deviation:
                    ...
                    print(
                        f"    ⚠️  {scheme.value} 延遲偏差過大: {avg_latency:.1f}ms vs 目標 {target_latency:.1f}ms"
                    )
    ...
    async def test_scheme_switching(self) -> bool:
        ...
        try:
            ...
            if len(switch_results) == len(switch_sequence):
                tests_passed += 1
                print(
                    f"  ✅ 換手序列完整性: {len(switch_results)}/{len(switch_sequence)}"
                )
            else:
                print(
                    f"  ❌ 換手序列不完整: {len(switch_results)}/{len(switch_sequence)}"
                )
            ...
            if correct_switches == len(switch_sequence):
                tests_passed += 1
                print(f"  ✅ 方案換手正確性: {correct_switches}/{len(switch_sequence)}")
            else:
                print(f"  ❌ 方案換手錯誤: {correct_switches}/{len(switch_sequence)}")
            ...
            for result in switch_results:
                ...
                if (
                    ...
                    and 20 <= actual_latency <= 35
                ):
                    continue
                elif (
                    scheme == self.HandoverScheme.NTN_GS
                    and 140 <= actual_latency <= 170
                ):
                    continue
                elif (
                    scheme == self.HandoverScheme.NTN_SMN
                    and 140 <= actual_latency <= 180
                ):
                    continue
                elif (
                    scheme == self.HandoverScheme.NTN_BASELINE
                    and 200 <= actual_latency <= 300
                ):
                    continue
                else:
            ...
            if avg_switch_time < 200:  # 每次換手 < 200ms
                tests_passed += 1
                print(f"  ✅ 換手效率良好: {avg_switch_time:.1f}ms/換手")
            else:
                print(f"  ❌ 換手效率過低: {avg_switch_time:.1f}ms/換手")
            ...
            success_rate = tests_passed / total_tests
            print(
                f"\n📊 方案換手測試結果: {tests_passed}/{total_tests} ({success_rate:.1%})"
            )

            self.test_results.append(("方案換手", success_rate >= 0.8))
            return success_rate >= 0.8

        except Exception as e:
            print(f"❌ 方案換手測試失敗: {e}")
            self.test_results.append(("方案換手", False))
            return False
    ...
    async def test_scheme_performance_isolation(self) -> bool:
        ...
        try:
            ...
            async def generate_scheme_events(
                scheme: self.HandoverScheme, count: int = 15
            ):
                events = []
                base_latency = {
            ...
            for scheme in concurrent_schemes:
                if scheme_event_counts.get(scheme, 0) == 15:
                    ...
                    print(
                        f"    ⚠️  {scheme.value} 事件數異常: {scheme_event_counts.get(scheme, 0)}"
                    )
    ...
    def generate_test_report(self):
        ...
        if self.scheme_metrics:
            ...
            if "switching" in self.scheme_metrics:
                ...
                print(f"   方案換手:")
                print(f"     - 換手次數: {switch_metrics['switch_count']}")
                print(
                    f"     - 平均換手時間: {switch_metrics['avg_switch_time_ms']:.1f}ms"
                )
        ...
        feature_completion = {
            "方案初始化功能": any(
                name == "方案初始化" and result for name, result in self.test_results
            ),
            "方案差異化功能": any(
                name == "方案差異化" and result for name, result in self.test_results
            ),
            "方案換手功能": any(
                name == "方案換手" and result for name, result in self.test_results
            ),
            "方案效能隔離": any(
                name == "方案效能隔離" and result for name, result in self.test_results
            ),
        }
