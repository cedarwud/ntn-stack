...
# 添加 NetStack API 路徑
sys.path.insert(
    0, str(Path(__file__).parent.parent.parent / "netstack" / "netstack_api")
)
...
class Comprehensive14Test:
    ...
    async def test_module_imports(self) -> bool:
        ...
        try:
            # 測試核心服務導入
            from netstack.netstack_api.services.paper_synchronized_algorithm import (
                SynchronizedAlgorithm,
            )
            from netstack.netstack_api.services.fast_access_prediction_service import (
    ...
    async def test_upf_extension(self) -> bool:
        ...
        try:
            # 檢查 UPF 擴展目錄結構
            upf_extension_dir = Path(
                "/home/sat/ntn-stack/netstack/docker/upf-extension"
            )
                ...
                try:
                    ...
                    if any(
                        ...
                        if response.status_code in [
                            ...
                            print(
                                f"    ⚠ API 端點異常: {endpoint} (status: {response.status_code})"
                            )
                    else:
                        # POST 端點檢查是否存在
    ...
    async def test_performance_measurement(self) -> bool:
        ...
        try:
            ...
            # 驗證基本功能
            if (
                len(event_ids) == len(test_events)
                and len(statistics) >= 2
                and "comparison_table" in report
            ):
                return True
            else:
                print("    ❌ 效能測量框架驗證失敗")
    ...
    async def test_cross_component_integration(self) -> bool:
        ...
        try:
            ...
            await fast_pred.register_ue(
                ue_id=test_ue_id, position={"lat": 25.0, "lon": 121.0, "alt": 100.0}
            )
            ...
            integration_success = (
                # 檢查同步演算法狀態存在
                (
                    algo_status.get("algorithm_state") is not None
                    or algo_status.get("performance_stats") is not None
                    or algo_status.get("is_running") is not None
                )
                and
                # 檢查快速預測服務正常
                pred_status.get("service_name") == "FastSatellitePrediction"
                and
                # 檢查測量統計有數據
                measurement_stats.get("total_events") > 0
                and
                # 檢查組件數量正常
