...
#### 🔍 為什麼需要獨立的信號處理階段？
1. **3GPP 標準合規性**：A4/A5/D2 事件需要精確的信號品質參數
2. **星座特定建模**：不同星座的信號特性差異巨大，需要分離處理
3. **事件參數預計算**：為後續換手決策提供標準化的事件判斷依據
4. **擴展性考量**：新增其他 3GPP events 只需調整此階段

### 📊 實際處理流程 (2025-08-10 最新架構)

```
階段2處理流程:
├── 📥 輸入: 8,715 顆衛星的完整軌道數據 (第1階段輸出)
├── 🔄 星座分離處理:
│   ├── Starlink: 8,064 顆 → 星座特定信號模型
│   └── OneWeb: 651 顆 → 星座特定信號模型
├── 🛰️ 信號品質計算:
│   ├── RSRP/RSRQ/SINR 實時計算
│   ├── 自由空間路徑損耗 (FSPL)
│   ├── 大氣衰減建模 (仰角相關)
...
> **🔄 重構說明**: 原本的第二階段內容將重新編號為第三階段，並整合第二階段的輸出結果。以下內容待修改以適應新架構。
...
> **🔄 修改說明**: 此階段需要整合第二階段(3GPP Events)的輸出結果，調整處理邏輯以適應新架構。
...
## ⚠️ 第五階段：最終輸出格式與存儲策略 *(待修改)*

> **🔄 修改說明**: 此階段需要整合第二階段(3GPP Events)的信號品質參數和事件判斷結果，實現混合存儲策略以適應新架構。

## 📁 第五階段：最終輸出格式與存儲策略 *(原第四階段)*
