...
#### 🔧 Debug Mode 控制機制 (v2.1 最終實現)
- **Debug Mode = True** (預設): 重新計算所有數據並生成 `stage1_tle_sgp4_output.json` 檔案 (2.2GB)
- **Debug Mode = False**: 即時處理模式，刪除任何現有檔案，重新計算最新數據，不保存檔案
- **數據一致性保證**: 無論模式如何，都使用最新TLE數據進行完整計算
- **存儲優化**: 即時模式避免2.2GB檔案持續存在，節省存儲空間
- **用途說明**: True適用於開發調試，False適用於生產環境的即時處理
...
### 處理規模
- **衛星總數**：8,715 顆（全量真實數據）
- **最終衛星池**：555 Starlink + 134 OneWeb (智能篩選結果)
