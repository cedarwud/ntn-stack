...
class DeviceService:
    ...
    async def create_device(self, device_data: DeviceCreate) -> Device:
        """創建新設備，並檢查名稱是否已存在"""
        # 檢查名稱是否已存在
        existing_device = await self.device_repository.get_by_name(name=device_data.name)
        if existing_device:
    ...
    async def update_device(self, device_id: int, device_data: DeviceUpdate) -> Device:
        ...
        # 更新設備
        return await self.device_repository.update_by_id(device_id=device_id, device_in=device_data)
    ...
    async def delete_device(self, device_id: int) -> Device:
        ...
        jammer_count = sum(1 for d in remaining_devices if d.role == DeviceRole.JAMMER.value)
