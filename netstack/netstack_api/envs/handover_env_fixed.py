...
class SatelliteState:
    ...
    beam_coverage: List[Tuple[float, float]] = field(
...
class LEOSatelliteHandoverEnv(gym.Env):
    ...
    def __init__(
        ...
        # 初始化真實數據適配器
        if RealDataAdapter is not None:
            self.real_data_adapter = RealDataAdapter(
                netstack_api_url=netstack_api_url,
                simworld_api_url=simworld_api_url,
                fallback_to_mock=True,
                timeout=5.0,
            )
            self.use_real_data = True
            logger.info("已啟用真實數據適配器")
        else:
            self.real_data_adapter = None
            self.use_real_data = False
            logger.warning("無法載入真實數據適配器，使用模擬數據")
    ...
    def _generate_mock_data(self) -> Dict[str, Any]:
        ...
        for i in range(min(5, self.max_ues)):
            ...
            ues[ue_id] = UEState(
                ...
                current_satellite=(
                    random.choice(list(satellites.keys())) if satellites else None
                ),
    ...
    async def _get_real_data(self) -> Dict[str, Any]:
        """獲取真實數據並轉換為環境格式"""
        real_data = await self.real_data_adapter.get_complete_real_data()

        # 轉換衛星數據格式
        satellites = {}
        for sat_id, sat_data in real_data.get("satellites", {}).items():
            satellites[sat_id] = SatelliteState(
                satellite_id=sat_data["satellite_id"],
                latitude=sat_data["latitude"],
                longitude=sat_data["longitude"],
                altitude=sat_data["altitude"],
                velocity=tuple(sat_data.get("velocity", [0.0, 0.0, 0.0])),
                elevation_angle=sat_data.get("elevation_angle", 0.0),
                azimuth_angle=sat_data.get("azimuth_angle", 0.0),
                distance=sat_data.get("distance", 0.0),
                load_factor=sat_data.get("load_factor", 0.5),
                available_bandwidth=sat_data.get("available_bandwidth", 100.0),
                beam_coverage=sat_data.get("beam_coverage", []),
                handover_count=sat_data.get("handover_count", 0),
                is_available=sat_data.get("is_available", True),
            )

        # 轉換 UE 數據格式
        ues = {}
        for ue_id, ue_data in real_data.get("ues", {}).items():
            ues[ue_id] = UEState(
                ue_id=ue_data["ue_id"],
                latitude=ue_data["latitude"],
                longitude=ue_data["longitude"],
                altitude=ue_data.get("altitude", 0.0),
                velocity=tuple(ue_data.get("velocity", [0.0, 0.0, 0.0])),
                current_satellite=ue_data.get("current_satellite"),
                signal_strength=ue_data.get("signal_strength", -80.0),
                sinr=ue_data.get("sinr", 15.0),
                throughput=ue_data.get("throughput", 50.0),
                latency=ue_data.get("latency", 20.0),
                packet_loss=ue_data.get("packet_loss", 0.1),
                battery_level=ue_data.get("battery_level", 100.0),
                service_type=ue_data.get("service_type", "eMBB"),
                qos_requirements=ue_data.get("qos_requirements", {}),
            )

        return {
            "satellites": satellites,
            "ues": ues,
            "handover": real_data.get(
                "handover", {"active_handovers": [], "total_count": 0}
            ),
            "data_source": real_data.get("data_source", "real_api"),
            "timestamp": real_data.get("timestamp"),
        }

    def _get_real_data_sync(self) -> Dict[str, Any]:
        """同步獲取真實數據並轉換為環境格式"""
        import asyncio
        ...
        try:
            ...
            # 如果有運行的循環，直接調用 get_complete_real_data 的同步版本
            real_data = self._get_real_data_fallback()
        except RuntimeError:
            # 沒有運行的事件循環，創建新的
            real_data = asyncio.run(self.real_data_adapter.get_complete_real_data())
    ...
    def _get_real_data_fallback(self) -> Dict[str, Any]:
        """當無法調用異步API時的回退方法"""
        # 由於API服務不可用，直接使用適配器的模擬數據方法
        satellites = self.real_data_adapter._generate_mock_satellite_data()
        ues = self.real_data_adapter._generate_mock_ue_data()
        network_env = self.real_data_adapter._generate_mock_network_data()
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.use_real_data and self.real_data_adapter:
            try:
                # 使用同步的數據獲取方法
                data = self._get_real_data_sync()
                logger.info(
                    f"成功獲取真實數據，來源: {data.get('data_source', 'unknown')}"
                )
            except Exception as e:
                logger.warning(f"獲取真實數據失敗，回退到模擬數據: {e}")
        ...
        logger.info(
            f"環境重置完成，UE數量: {len(self.ue_states)}, 衛星數量: {len(self.satellite_states)}"
        )
    ...
    def step(self, action):
        ...
        observation = self._get_observation()
        info = self._get_info()
        info.update({"handover_results": handover_results, "step": self.current_step})
        ...
        for action in handover_actions:
            ...
            if success:
                ...
                if action.ue_id in self.ue_states:
                    self.ue_states[action.ue_id].current_satellite = (
                        action.target_satellite
                    )
                    # 模擬信號品質改善
    ...
    def _simulate_handover_latency(self, action: HandoverAction) -> float:
        ...
        total_latency = (
            base_latency + timing_penalty + load_penalty + congestion_penalty
        )
    ...
    def _update_environment(self):
        ...
        # 更新網路環境
        self.network_environment.interference_level = max(
            0.0,
            min(
                1.0,
        ...
        self.network_environment.network_congestion = max(
            0.0,
            min(
                1.0,
    ...
    def _calculate_reward(
        self, actions: List[HandoverAction], results: List[Dict]
    ) -> float:
        """計算獎勵函數"""
        total_reward = 0.0
        ...
        for action, result in zip(actions, results):
            ...
            if result["success"]:
                ...
                total_reward += (
                    latency_reward + sinr_reward + throughput_reward + timing_reward
                )
        ...
        # 過多切換的懲罰
        handover_rate = len(
            [a for a in actions if a.action_type != "no_handover"]
        ) / max(1, len(actions))
        ...
        if self.satellite_states:
            load_variance = np.var(
                [s.load_factor for s in self.satellite_states.values()]
            )
    ...
    def _get_info(self) -> Dict[str, Any]:
        ...
        average_latency = self.total_handover_latency / max(
            1, self.successful_handovers
        )

        avg_sinr = 0.0
        if self.ue_states:
            avg_sinr = sum(ue.sinr for ue in self.ue_states.values()) / len(
                self.ue_states
            )
    ...
    def render(self, mode="human"):
        ...
        if mode == "console":
            ...
            print(
                f"Success Rate: {self.successful_handovers/max(1,self.total_handovers):.2%}"
            )
