...
async def test_tle_validation(satellites):
    ...
    try:
        ...
        ts = load.timescale()
        earth = ts.utc(2024, 7, 27, 12, 0, 0)  # 使用固定時間
        ...
        for i, sat_data in enumerate(satellites):
            try:
                ...
                # 檢查軌道高度合理性
                if 200 <= distance <= 2000:
                    valid_count += 1
                    logger.debug(f"✅ {sat_data['name']}: 高度 {distance:.1f} km")
                else:
                    logger.warning(f"⚠️ {sat_data['name']}: 軌道高度異常 {distance:.1f} km")
...
async def test_visibility_calculation(candidates):
    ...
    try:
        ...
        # 測試30分鐘時間窗
        base_time = ts.utc(2024, 7, 27, 12, 0, 0)  # 使用固定時間
        time_points = []
        for i in range(30):  # 30分鐘，每分鐘一個點
            t = ts.utc(2024, 7, 27, 12, i, 0)
