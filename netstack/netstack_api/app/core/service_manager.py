...
class ServiceManager:
    ...
    def __init__(self, mongo_adapter: MongoAdapter, redis_adapter: RedisAdapter, open5gs_adapter: Open5GSAdapter):
        """
        初始化服務管理器
    ...
    async def initialize_services(self, app: FastAPI) -> None:
        ...
        try:
            ...
            app.state.slice_service = SliceService(self.mongo_adapter, self.open5gs_adapter, self.redis_adapter)
            logger.info("✅ Slice 服務初始化完成")
            
            app.state.health_service = HealthService(self.mongo_adapter, self.redis_adapter)
    ...
    def get_service_status(self, app: FastAPI) -> dict:
        ...
        """
        services = [
            "ue_service", "slice_service", "health_service", "ueransim_service",
            "satellite_service", "sionna_service", "interference_service",
            "connection_service", "mesh_service", "uav_failover_service"
        ]
    ...
    async def check_all_services(app: FastAPI) -> dict:
        ...
        try:
            if hasattr(app.state, 'health_service'):
                ...
                results["adapters"] = {"status": "unknown", "message": "健康服務未初始化"}
        except Exception as e:
        ...
        service_checks = {}
        services = [
            "ue_service", "slice_service", "connection_service", 
            "mesh_service", "uav_failover_service"
        ]
