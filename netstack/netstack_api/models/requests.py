...
class SliceSwitchRequest(BaseModel):
    ...
    target_slice: str = Field(..., description="目標 Slice 類型", example="uRLLC")
...
class UERegistrationRequest(BaseModel):
    ...
    key: str = Field(
        ..., description="UE 的 K 金鑰", example="465B5CE8B199B49FAA5F0A2EE238A6BC"
    )

    opc: str = Field(
        ..., description="UE 的 OPc 值", example="E8ED289DEBA952E4283B54E88E6183CA"
    )

    apn: str = Field(default="internet", description="接入點名稱", example="internet")

    slice_type: str = Field(..., description="預設 Slice 類型", example="eMBB")
...
class UEUpdateRequest(BaseModel):
    ...
    apn: Optional[str] = Field(None, description="接入點名稱", example="internet")

    slice_type: Optional[str] = Field(None, description="Slice 類型", example="uRLLC")
