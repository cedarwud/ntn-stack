...
class RedisAdapter:
    ...
    async def cache_ue_info(
        self, imsi: str, ue_info: Dict[str, Any], ttl: int = 300
    ) -> None:
        """
        快取 UE 資訊
        ...
        try:
            key = f"ue:info:{imsi}"
            await self.client.setex(key, ttl, json.dumps(ue_info, default=str))
    ...
    async def update_ue_stats(self, imsi: str, stats: Dict[str, Any]) -> None:
        """
        更新 UE 統計資料
    ...
    async def record_slice_switch(
        self, imsi: str, from_slice: str, to_slice: str
    ) -> None:
        """
        記錄 Slice 換手事件
        ...
        try:
            ...
            await self.client.lpush(history_key, json.dumps(switch_record))
            ...
            logger.info(
                "Slice 換手已記錄", imsi=imsi, from_slice=from_slice, to_slice=to_slice
            )
        except Exception as e:
            logger.error(
                "記錄 Slice 換手失敗",
    ...
    async def get_slice_switch_history(
        ...
        try:
            ...
            logger.error("取得 Slice 換手歷史失敗", imsi=imsi, error=str(e))
            return []

    async def update_rtt_measurement(
        self, imsi: str, rtt_ms: float, slice_type: str
    ) -> None:
        """
        更新 RTT 測量結果
        ...
        try:
            ...
            await self.client.hset(
                stats_key,
                mapping={"rtt_ms": rtt_ms, "last_rtt_test": datetime.now().isoformat()},
            )
            ...
            logger.debug(
                "RTT 測量已更新", imsi=imsi, rtt_ms=rtt_ms, slice_type=slice_type
            )
        except Exception as e:
            logger.error(
                "更新 RTT 測量失敗",
