...
class UnifiedAIService:
    ...
    async def make_decision(
        self, input_data: Dict[str, Any], context: Optional[Dict] = None
    ) -> Dict[str, Any]:
        """統一決策接口"""
    ...
    async def learn(self, experience: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug("學習完成", service_name=self.service_name, result=result)
    ...
    async def train(self, episodes: int = 1000) -> Dict[str, Any]:
        """訓練接口"""
        try:
            logger.info("開始訓練", service_name=self.service_name, episodes=episodes)

            result = await self.engine.train(episodes)

            logger.info("訓練完成", service_name=self.service_name, result=result)
    ...
    def _preprocess_input(
        ...
        if "position" in processed and not isinstance(
            processed["position"], (list, tuple)
        ):
    ...
    def _postprocess_output(
        self, action: Dict[str, Any], original_input: Dict[str, Any]
    ) -> Dict[str, Any]:
        """後處理輸出結果"""
    ...
    async def _fallback_decision(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "interference" in self.service_name.lower():
            ...
            return {"action": "maintain_formation", "fallback": True}
        else:
            return {"action": "no_change", "fallback": True}
    ...
    async def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_response_time": sum(self.response_times[-10:])
            / max(1, len(self.response_times[-10:])),
...
class ServiceContainer:
    ...
    def register_service(
        ...
        service = UnifiedAIService(
            service_name=service_name, engine=engine, config=merged_config
        )
