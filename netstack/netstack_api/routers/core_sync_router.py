...
if not UPF_BRIDGE_AVAILABLE:
    ...
    class MockHandoverRequest:
        def __init__(
            self, ue_id: str, target_satellite_id: str, predicted_time: float, **kwargs
        ):
            self.ue_id = ue_id
...
class SyncPredictRequest(BaseModel):
    ...
    algorithm: str = Field(
        "algorithm_1", description="使用的演算法 (algorithm_1 或 algorithm_2)"
    )
...
class SyncHandoverRequest(BaseModel):
    ...
    predicted_time: Optional[float] = Field(None, description="預測換手時間戳")
    force_handover: bool = Field(False, description="強制執行換手")
...
class UPFIntegrationRequest(BaseModel):
    ...
    access_strategy: str = Field(
        "flexible", description="接入策略 (flexible 或 consistent)"
    )
    position: Optional[Dict[str, float]] = Field(
        None, description="UE 位置 {lat, lon, alt}"
    )
...
def get_core_sync_service() -> CoreNetworkSyncService:
    ...
    if core_sync_service is None:
        ...
        core_sync_service = CoreNetworkSyncService(
            fine_grained_sync_service=fine_grained_sync, event_bus_service=event_bus
        )
...
def get_paper_algorithm() -> SynchronizedAlgorithm:
    ...
    if paper_algorithm is None:
        paper_algorithm = SynchronizedAlgorithm(
            delta_t=5.0, binary_search_precision=0.01
        )
...
async def start_core_sync_service(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"啟動核心同步服務失敗: {str(e)}")


@router.post(
    "/service/stop", summary="停止核心同步服務", description="停止核心網路同步服務"
)
async def stop_core_sync_service(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"停止核心同步服務失敗: {str(e)}")
...
async def get_sync_status_summary(
    ...
    try:
        ...
        summary = {
            ...
            "signaling_free_enabled": status["sync_performance"][
                "signaling_free_enabled"
            ],
            "synchronized_components": len(
                [
                    comp
                    for comp, info in status["component_states"].items()
        ...
        raise HTTPException(status_code=500, detail=f"獲取同步狀態摘要失敗: {str(e)}")
...
async def get_component_sync_status(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail=f"無效的組件名稱。有效選項: {valid_components}"
            )
        ...
        if not component_info:
            raise HTTPException(
                status_code=404, detail=f"找不到組件 {component_name} 的狀態信息"
            )
        ...
        raise HTTPException(status_code=500, detail=f"獲取組件狀態失敗: {str(e)}")
...
async def sync_component(
    ...
    try:
        ...
        if sync_result["success"]:
            ...
            logger.error(
                f"組件 {request.component} 同步失敗: {sync_result.get('error')}"
            )
        ...
        raise HTTPException(status_code=500, detail=f"同步組件失敗: {str(e)}")


@router.post(
    "/prediction/satellite-access",
    summary="預測衛星接入",
...
async def predict_satellite_access(
    ...
    try:
        if not service.fine_grained_sync:
            raise HTTPException(status_code=503, detail="Fine-Grained 同步服務不可用")
        ...
        raise HTTPException(status_code=500, detail=f"衛星接入預測失敗: {str(e)}")
...
async def emergency_resync(
    ...
    try:
        ...
        if not target_components:
            raise HTTPException(status_code=400, detail="沒有有效的目標組件")
        ...
        raise HTTPException(status_code=500, detail=f"緊急重新同步失敗: {str(e)}")
...
async def get_performance_metrics(
    ...
    try:
        if component:
            ...
            if not metrics:
                raise HTTPException(
                    status_code=404, detail=f"找不到組件 {component} 的性能指標"
                )
            ...
            return {
                ...
                "summary": {
                    ...
                    "average_accuracy_ms": sum(
                        m["sync_accuracy_ms"] for m in all_metrics.values()
                    )
                    / len(all_metrics),
                    "average_availability": sum(
                        m["availability"] for m in all_metrics.values()
                    )
                    / len(all_metrics),
                },
                "timestamp": datetime.now().isoformat(),
...
@router.get(
    "/events/recent", summary="獲取最近同步事件", description="獲取最近的同步事件列表"
)
async def get_recent_sync_events(
    ...
    try:
            ...
            formatted_events.append(
                {
                    ...
                    "target_component": (
                        event.target_component.value if event.target_component else None
                    ),
        ...
        raise HTTPException(status_code=500, detail=f"獲取同步事件失敗: {str(e)}")


# Health check endpoint
@router.get("/health", summary="健康檢查", description="檢查核心同步服務的健康狀態")
async def health_check(
    service: CoreNetworkSyncService = Depends(get_core_sync_service),
):
    """健康檢查"""
    try:
        is_healthy = service.is_running and service.core_sync_state in [
            CoreSyncState.SYNCHRONIZED,
        ...
        health_info = {
            ...
            "active_tasks": len(
                [t for t in service.sync_tasks.values() if not t.done()]
            ),
    ...
    try:
        ...
        if request.algorithm == "algorithm_1":
            ...
            predictions = await fast_service.predict_access_satellites(
                users=request.ue_ids, satellites=satellites, time_t=future_time
            )
        ...
        raise HTTPException(status_code=500, detail=f"同步預測失敗: {str(e)}")


@router.post(
    "/sync/handover", summary="手動觸發換手", description="手動觸發 UE 衛星換手"
)
async def trigger_sync_handover(
    request: SyncHandoverRequest, upf_bridge: UPFSyncBridge = Depends(get_upf_bridge)
):
    """手動觸發換手"""
    try:
        ...
        if success:
            ...
            raise HTTPException(status_code=500, detail="UPF 換手執行失敗")

    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"觸發換手失敗: {e}")
        raise HTTPException(status_code=500, detail=f"觸發換手失敗: {str(e)}")


@router.get(
    "/sync/status", summary="獲取演算法運行狀態", description="獲取同步演算法的運行狀態"
)
async def get_sync_status(
    ...
    try:
        ...
        return {
            ...
            "overall_status": {
                "algorithms_running": (
                    algorithm1_status.get("algorithm_state") != "stopped"
                    or algorithm2_status.get("accuracy_achieved", False)
                ),
...
async def get_sync_metrics(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"獲取同步指標失敗: {str(e)}")
...
async def register_ue_to_upf(
    request: UPFIntegrationRequest, upf_bridge: UPFSyncBridge = Depends(get_upf_bridge)
):
    """註冊 UE 到 UPF"""
    try:

        if not upf_bridge:
            raise HTTPException(status_code=503, detail="UPF 橋接服務不可用")

        # 創建 UE 資訊
        access_strategy = (
            AccessStrategy.FLEXIBLE
            if request.access_strategy == "flexible"
        ...
        ue_info = UEInfo(
            ...
            or {"lat": 24.1477, "lon": 120.6736, "alt": 100.0},
        )
        ...
        if success:
            ...
            raise HTTPException(status_code=500, detail="UE 註冊失敗")

    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"註冊 UE 到 UPF 失敗: {e}")
        raise HTTPException(status_code=500, detail=f"註冊 UE 失敗: {str(e)}")
...
async def get_ue_upf_status(
    ...
    try:
        ...
        if ue_status is None:
            raise HTTPException(status_code=404, detail=f"找不到 UE {ue_id}")
        ...
        raise HTTPException(status_code=500, detail=f"獲取 UE 狀態失敗: {str(e)}")
...
@router.post(
    "/measurement/record-handover",
    summary="記錄換手事件",
    description="記錄換手事件到效能測量系統",
)
async def record_handover_event(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"無效的換手方案: {scheme}. 有效選項: {valid_schemes}",
            )
        ...
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"無效的換手結果: {result}. 有效選項: {valid_results}",
            )
        ...
        logger.error(f"記錄換手事件失敗: {e}")
        raise HTTPException(status_code=500, detail=f"記錄換手事件失敗: {str(e)}")
...
async def get_measurement_statistics(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"獲取效能統計失敗: {str(e)}")
...
async def get_comparison_report(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"生成對比報告失敗: {str(e)}")


@router.post(
    "/measurement/generate-cdf",
    summary="生成 CDF 圖表",
    description="生成換手延遲的累積分佈函數圖表",
)
async def generate_cdf_plot(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"生成 CDF 圖表失敗: {str(e)}")


@router.post(
    "/measurement/export-data", summary="匯出測量數據", description="匯出效能測量數據"
)
async def export_measurement_data(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"匯出測量數據失敗: {str(e)}")


@router.post(
    "/measurement/run-automated-test",
    summary="執行自動化對比測試",
...
async def run_automated_comparison_test(
    ...
    try:
        ...
        if duration_seconds < 10 or duration_seconds > 3600:
            raise HTTPException(
                status_code=400, detail="測試持續時間必須在 10-3600 秒之間"
            )

        if ue_count < 1 or ue_count > 20:
            raise HTTPException(status_code=400, detail="UE 數量必須在 1-20 之間")

        if handover_interval_seconds < 1.0 or handover_interval_seconds > 60.0:
            raise HTTPException(
                status_code=400, detail="換手間隔必須在 1.0-60.0 秒之間"
            )
        ...
        raise HTTPException(status_code=500, detail=f"執行自動化測試失敗: {str(e)}")
