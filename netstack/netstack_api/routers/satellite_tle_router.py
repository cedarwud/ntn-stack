...
class OrbitPredictionRequest(BaseModel):
    ...
    observer_lon: Optional[float] = Field(
        None, ge=-180, le=180, description="觀測者經度"
    )
...
class HandoverPredictionRequest(BaseModel):
    ...
    candidate_satellites: List[str] = Field(..., description="候選換手衛星列表")
...
class BinarySearchHandoverRequest(BaseModel):
    ...
    precision_seconds: Optional[float] = Field(
        0.01, ge=0.001, le=1.0, description="搜尋精度（秒）"
    )
...
def get_satellite_service() -> SatelliteGnbMappingService:
    ...
    if satellite_service is None:
        ...
        satellite_service = SatelliteGnbMappingService(
            simworld_api_url=simworld_url, redis_client=redis_client
        )
...
def get_tle_bridge_service() -> SimWorldTLEBridgeService:
    ...
    if (
        tle_bridge_service is None
        or tle_bridge_service.simworld_api_url != current_simworld_url
    ):
        # 這裡可以配置 Redis 連接
        redis_client = None  # 可以根據需要配置

        tle_bridge_service = SimWorldTLEBridgeService(
            simworld_api_url=current_simworld_url, redis_client=redis_client
        )
    ...
    try:
        ...
        return {
            ...
            "statistics": {
                ...
                "failed_count": failed_count,
                "success_rate": (
                    success_count / len(request.satellite_ids)
...
@router.post(
    "/handover/predict", summary="預測換手時機", description="預測 UE 的衛星換手時機"
)
async def predict_handover_timing(
    ...
    try:
        ...
        ue_position = UAVPosition(
            latitude=request.ue_lat, longitude=request.ue_lon, altitude=request.ue_alt
        )
        ...
        logger.error("換手時機預測失敗", error=str(e), ue_id=request.ue_id)
        raise HTTPException(status_code=500, detail=f"換手時機預測失敗: {str(e)}")


@router.post(
    "/handover/binary-search",
    summary="二分搜尋換手時間",
    description="使用二分搜尋算法計算精確的換手時間點",
)
async def binary_search_handover_time(
    ...
    try:
        ...
        return {
            ...
            "search_info": {
                "search_start": datetime.fromtimestamp(
                    request.search_start_timestamp
                ).isoformat(),
                "search_end": datetime.fromtimestamp(
                    request.search_end_timestamp
                ).isoformat(),
        ...
        logger.error("二分搜尋換手時間失敗", error=str(e), ue_id=request.ue_id)
        raise HTTPException(status_code=500, detail=f"二分搜尋換手時間失敗: {str(e)}")
...
@router.post(
    "/tle/sync", summary="同步 TLE 資料", description="從 SimWorld 同步最新的 TLE 資料"
)
...
@router.get(
    "/status", summary="服務狀態總覽", description="獲取 TLE 橋接服務的整體狀態"
)
async def get_service_status(
    ...
    try:
        ...
        status_info = {
            ...
            "overall_health": {
                "healthy": (
                    satellite_service_available
                    and bridge_service_available
                    and tle_health.get("success", False)
                ),
                "services_online": (
                    int(satellite_service_available) + int(bridge_service_available)
                ),
                "total_services": 2,
            },
            "timestamp": datetime.utcnow().isoformat(),
...
@router.get(
    "/health", summary="簡單健康檢查", description="檢查 TLE 橋接服務的基本健康狀態"
)
