...
async def get_event_bus() -> EventBusService:
    ...
    # 暫時禁用事件總線以避免協程錯誤
    raise RuntimeError("事件總線已暫時禁用以避免協程錯誤")
...
class HandoverEventBusExtension:
    ...
    async def subscribe_to_handover_events(
        ...
        for pattern in event_patterns:
            ...
            if pattern.endswith(".*"):
                ...
                matching_types = [
                    event_type
                    for event_type in dir(HandoverEventTypes)
                    if not event_type.startswith("_")
                    and getattr(HandoverEventTypes, event_type).startswith(base_pattern)
                ]
                ...
                for event_type_name in matching_types:
                    ...
                    handler_id = self.event_bus.register_handler(
                        ...
                        handler_func=handler_func,
                        handler_id=(
                            f"{subscriber_id}_{event_type}" if subscriber_id else None
                        ),
                        priority=priority,
                    )
    ...
    async def publish_metrics_event(
        self, metrics_data: Dict[str, Any], priority: str = "LOW"
    ) -> str:
        """發布換手指標事件"""
    ...
    async def publish_alert_event(
        self, alert_data: Dict[str, Any], priority: str = "HIGH"
    ) -> str:
        """發布換手性能警報事件"""
    ...
    async def get_handover_event_history(
        ...
        if event_type:
            events = await self.event_bus.event_store.get_events_by_type(
                event_type, limit
            )
        else:
            # 獲取所有換手相關事件
...
def handover_event_handler(
    event_patterns: List[str], priority: int = 100, subscriber_id: Optional[str] = None
):
    """換手事件處理器裝飾器"""
