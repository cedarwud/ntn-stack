...
class AlgorithmIntegrationBridge:
    ...
    async def start_integrated_algorithms(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 根據模式啟動相應的演算法
            if self.current_mode in [IntegrationMode.PAPER_ONLY, IntegrationMode.HYBRID, IntegrationMode.FALLBACK]:
                paper_result = await self.paper_algorithm.start_algorithm()
                start_results["paper_algorithm"] = paper_result
            
            if self.current_mode in [IntegrationMode.ENHANCED_ONLY, IntegrationMode.HYBRID, IntegrationMode.FALLBACK]:
                if self.enhanced_algorithm:
                    await self.enhanced_algorithm.start_enhanced_algorithm()
                    start_results["enhanced_algorithm"] = {"success": True, "message": "Enhanced algorithm started"}
    ...
    async def stop_integrated_algorithms(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.enhanced_algorithm:
                ...
                stop_results["enhanced_algorithm"] = {"success": True, "message": "Enhanced algorithm stopped"}
    ...
    async def _predict_handover_paper_only(
        ...
        # 獲取當前和預測的接入衛星
        current_access = await self.paper_algorithm.calculate_access_satellite(ue_id, current_time)
        predicted_access = await self.paper_algorithm.calculate_access_satellite(ue_id, target_time)
        ...
        return {
            ...
            "handover_time": datetime.fromtimestamp(handover_time).isoformat() if handover_time else None,
            "time_to_handover_seconds": handover_time - current_time if handover_time else None,
        ...
        if self.config.two_point_prediction_enabled:
            ...
            if self.config.enhanced_binary_search_enabled:
                ...
                return {
                    ...
                    "ue_id": ue_id,
                    "current_satellite": current_satellite,
                    "predicted_satellite": two_point_result.prediction_t_delta.get("predicted_access_time"),
                    "handover_needed": True,
                    "handover_time": search_result["final_estimate"].isoformat(),
                    "time_to_handover_seconds": (search_result["final_estimate"] - datetime.utcnow()).total_seconds(),
    ...
    async def _predict_handover_hybrid(
        ...
        try:
            ...
            if self.enhanced_algorithm and self.config.enhanced_features_enabled:
                ...
                if enhanced_result.get("success") and enhanced_result.get("prediction_accuracy", 0) >= 0.8:
            ...
            # 如果兩種方法都可用，可以進行結果融合
            if self.enhanced_algorithm:
                paper_result["fusion_note"] = "Enhanced algorithm available but paper standard used due to confidence threshold"
            ...
            if self.enhanced_algorithm:
                try:
                    enhanced_result = await asyncio.wait_for(
                        self._predict_handover_enhanced_only(ue_id, current_satellite, time_horizon_seconds),
                    ...
                    self.logger.warning("進階演算法超時，切換到備援模式")
                    self.performance_metrics["fallback_activations"] += 1
                except Exception as e:
                    self.logger.warning("進階演算法失敗，切換到備援模式", error=str(e))
                    self.performance_metrics["fallback_activations"] += 1
    ...
    async def switch_integration_mode(self, new_mode: IntegrationMode) -> Dict[str, Any]:
        """
        切換整合模式
        ...
        try:
            ...
            self.logger.error("切換整合模式失敗", error=str(e))
            return {"success": False, "error": str(e)}
    ...
    async def get_integration_status(self) -> Dict[str, Any]:
        ...
        if self.enhanced_algorithm:
            try:
                enhanced_status = await self.enhanced_algorithm.get_algorithm_performance_report()
    ...
    def _update_performance_metrics(self, response_time_ms: float, success: bool):
        ...
        self.performance_metrics["last_24h_stats"] = [
            stat for stat in self.performance_metrics["last_24h_stats"]
