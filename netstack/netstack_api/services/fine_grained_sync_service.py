...
class FineGrainedSyncService:
    ...
    def __init__(self, satellite_service=None, orbit_service=None, 
                 handover_prediction_service=None, event_bus_service=None):
    ...
    async def predict_satellite_access(self, ue_id: str, satellite_id: str, 
                                     time_horizon_minutes: float = 30.0) -> SatelliteAccessPrediction:
        """
        預測衛星接入時間
        ...
        try:
            # 步驟1: 二點預測方法
            time_t = datetime.now()
            time_t_delta = time_t + timedelta(minutes=self.prediction_time_delta_minutes)
            
            # 在時間點 T 的預測
            prediction_t = await self._predict_at_time_point(ue_id, satellite_id, time_t)
            
            # 在時間點 T+Δt 的預測
            prediction_t_delta = await self._predict_at_time_point(ue_id, satellite_id, time_t_delta)
            
            # 步驟2: Binary Search Refinement
            initial_estimate = self._interpolate_predictions(prediction_t, prediction_t_delta)
    ...
    async def _predict_at_time_point(self, ue_id: str, satellite_id: str, 
                                   time_point: datetime) -> Dict[str, Any]:
        """在指定時間點進行預測"""
        try:
            # 獲取衛星軌道參數
            orbital_params = await self._get_satellite_orbital_params(satellite_id, time_point)
    ...
    def _interpolate_predictions(self, prediction_t: Dict, prediction_t_delta: Dict) -> Dict[str, Any]:
        ...
        # 線性內插計算初始估計
        time_diff = (prediction_t_delta["time_point"] - prediction_t["time_point"]).total_seconds()
        access_diff = (access_time_t_delta - access_time_t).total_seconds()
        
        # 計算斜率並外推到當前時間
        current_time = datetime.now()
        extrapolation_factor = (current_time - prediction_t["time_point"]).total_seconds() / time_diff
        
        interpolated_access_time = access_time_t + timedelta(seconds=access_diff * extrapolation_factor)
    ...
    async def _binary_search_refinement(self, ue_id: str, satellite_id: str, 
                                      initial_estimate: Dict, time_horizon_minutes: float) -> Dict[str, Any]:
        """
        二進制搜索精化算法
        ...
        try:
            while (search_state.iteration_count < search_state.max_iterations and 
                   search_state.current_error_ms > search_state.target_precision_ms):
                
                search_state.iteration_count += 1
                
                # 計算中點
                time_diff = (search_state.upper_bound - search_state.lower_bound).total_seconds()
                ...
                # 更新搜索範圍
                if access_evaluation["access_probability"] >= self.min_access_probability:
                    # 接入可能性高，搜索更早的時間
                ...
                # 檢查收斂條件
                if (time_diff < 10.0 or  # 搜索範圍小於10秒
                    search_state.current_error_ms <= search_state.target_precision_ms):  # 或達到目標精度
            ...
            return {
                ...
                "search_range_seconds": (search_state.upper_bound - search_state.lower_bound).total_seconds()
            }
    ...
    async def _evaluate_access_feasibility(self, ue_id: str, satellite_id: str, 
                                         candidate_time: datetime) -> Dict[str, Any]:
        """評估在候選時間點的接入可行性"""
        try:
            # 獲取候選時間點的衛星位置
            satellite_position = await self._get_satellite_position_at_time(satellite_id, candidate_time)
            ...
            access_probability = (elevation_factor + distance_factor + signal_factor) / 3.0
    ...
    def _estimate_timing_error(self, geometry: Dict, candidate_time: datetime) -> float:
        ...
        # 總誤差（毫秒）- 大幅降低基礎誤差
        total_error_ms = min(100.0, (timing_error_s + geometric_error_s * 0.05) * 1000.0)
    ...
    async def _calculate_prediction_confidence(self, prediction_t: Dict, 
                                             prediction_t_delta: Dict, 
                                             refined_prediction: Dict) -> float:
        """計算預測信心度"""
        # 基於多個因素計算信心度
        ...
        access_time_t = prediction_t["predicted_access_time"]
        access_time_t_delta = prediction_t_delta["predicted_access_time"]
        time_consistency = 1.0 - min(1.0, abs((access_time_t_delta - access_time_t).total_seconds()) / 3600.0)
    ...
    async def _perform_periodic_sync(self):
        ...
        try:
            ...
            if any(abs(drift) > self.max_clock_drift_ms for drift in clock_drift.values()):
                # 需要重新同步
    ...
    async def get_sync_status(self) -> Dict[str, Any]:
        """獲取同步狀態"""
        active_predictions = len([p for p in self.access_predictions.values() 
        ...
        if self.sync_statistics["successful_predictions"] > 0:
            recent_predictions = [
                p for p in self.access_predictions.values()
                if (datetime.now() - p.created_at).total_seconds() < 3600
            ]
            avg_accuracy = np.mean([p.error_bound_ms for p in recent_predictions]) if recent_predictions else 0.0
        ...
        return {
            ...
            "algorithm_performance": {
                "two_point_prediction_accuracy": self.sync_statistics.get("average_accuracy_ms", 0.0),
                "binary_search_convergence_rate": self.sync_statistics.get("convergence_rate", 0.0),
                "signaling_free_sync_status": "active" if self.is_running else "inactive"
            }
        }

    # 輔助方法（模擬實現）
    async def _get_satellite_orbital_params(self, satellite_id: str, time_point: datetime) -> Dict[str, Any]:
        """獲取衛星軌道參數"""
        # 模擬軌道參數
        return {
    ...
    async def _get_ue_position(self, ue_id: str, time_point: datetime) -> Dict[str, Any]:
        """獲取UE位置"""
        # 模擬UE位置
        return {
            "latitude": 25.0,
    ...
    async def _calculate_access_geometry(self, orbital_params: Dict, 
                                       ue_position: Dict, time_point: datetime) -> Dict[str, Any]:
        """計算接入幾何關係"""
        # 簡化的幾何計算
        return {
            "elevation_deg": 45.0,
    ...
    async def _compute_access_time(self, geometry: Dict, reference_time: datetime) -> datetime:
        """計算接入時間"""
        # 基於幾何參數計算最優接入時間
    ...
    async def _get_satellite_position_at_time(self, satellite_id: str, 
    ...
    async def _calculate_instantaneous_geometry(self, satellite_pos: Dict, 
                                              ue_pos: Dict, time_point: datetime) -> Dict[str, Any]:
        """計算瞬時幾何關係"""
        return {
            "elevation": 35.0,
            "azimuth": 180.0,
    ...
    async def _calculate_final_access_probability(self, ue_id: str, satellite_id: str, 
                                                access_time: datetime) -> float:
        """計算最終接入概率"""
        # 綜合評估接入概率
    ...
    async def _process_pending_predictions(self):
        ...
        current_time = datetime.now()
        expired_predictions = [
            pred_id for pred_id, pred in self.access_predictions.items()
