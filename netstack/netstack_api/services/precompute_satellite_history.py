...
class SatelliteHistoryPrecomputer:
    ...
    def __init__(self, tle_data: List[Dict], observer_coords: Tuple[float, float, float], 
    ...
    def compute_history(self, time_interval_seconds: int = 30) -> List[PrecomputeResult]:
        """
        預計算指定時間範圍內的所有衛星位置
        ...
        while current_time <= end_time:
            ...
            if processed_points % 100 == 0:
                ...
                self.logger.info(f"預計算進度: {progress:.1f}%", processed_points=processed_points)
    ...
    def compute_visible_satellites(self, timestamp: datetime) -> List[PrecomputeResult]:
        ...
        for sat_data in self.satellites:
            try:
                # SGP4 軌道計算
                error_code, position_teme, velocity_teme = sat_data['satrec'].sgp4(jd, fr)
                ...
                if elevation >= -10.0:
                    ...
                    result = PrecomputeResult(
                        ...
                        satellite_position={
                            ...
                            "azimuth": azimuth,
                            "range": distance,
                            "velocity": math.sqrt(sum(v**2 for v in velocity_teme))  # 速度大小
                        },
    ...
    def _teme_to_geodetic(self, position_teme: Tuple[float, float, float], 
    ...
    def _calculate_look_angles(self, sat_position: Position, 
                              observer: ObserverPosition) -> Tuple[float, float, float]:
        """
        計算衛星相對觀測者的仰角、方位角和距離
        ...
        # 高度差
        altitude_diff = sat_position.altitude - (observer.altitude / 1000.0)  # 轉換為 km
        ...
        x = (math.cos(observer_lat_rad) * math.sin(sat_lat_rad) - 
             math.sin(observer_lat_rad) * math.cos(sat_lat_rad) * math.cos(lon_diff))
    ...
    def _calculate_signal_quality(self, distance: float, elevation: float) -> Dict[str, float]:
        """
        計算信號品質指標
        ...
        return {
            ...
            "sinr": round(sinr, 2),
            "link_margin": round(max(0, signal_strength + 90), 2)  # 假設接收靈敏度 -90 dBm
        }
