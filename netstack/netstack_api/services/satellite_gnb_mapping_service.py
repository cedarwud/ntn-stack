...
class SatelliteGnbMappingService:
    ...
    def __init__(self, simworld_api_url: str = "http://simworld-backend:8000", redis_client: Optional[redis.Redis] = None):
    ...
    async def convert_satellite_to_gnb_config(
        ...
        try:
            ...
            self.logger.error("衛星位置轉換失敗", error=str(e), satellite_id=satellite_id)
            raise HTTPException(
                status_code=500,
    ...
    async def _get_satellite_position_from_simworld(
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                if observer_position:
                    params.update({
                        ...
                        "observer_alt": observer_position.altitude / 1000  # 轉為公里
                    })
            ...
            return await self._calculate_position_locally(satellite_id, observer_position)
    ...
    async def _calculate_ecef_coordinates(self, satellite_position: Dict) -> Dict[str, float]:
        """計算衛星的 ECEF 坐標"""
    ...
    def _convert_to_radio_parameters(
        ...
        if uav_position and satellite_position.get("range_km"):
            ...
            path_loss_db = 20 * math.log10(distance_km * 1000) + 20 * math.log10(frequency_hz) + 32.45
            ...
            base_power = 23
            distance_penalty = min(20, distance_km / 100)  # 每100km增加1dB損失，最大20dB
            ...
            radio_params.update({
                ...
                "link_budget_margin": max(0, tx_power - path_loss_db + 120),  # 假設接收靈敏度-120dBm
            })
        else:
            # 默認參數（無 UAV 位置時）
            radio_params.update({
    ...
    def _calculate_beam_coverage(self, altitude_km: float) -> Dict[str, float]:
        ...
        # 計算地平線距離
        horizon_distance_km = math.sqrt(altitude_km * (altitude_km + 2 * earth_radius_km))
    ...
    async def update_gnb_positions_continuously(
        ...
        while True:
            try:
                # 批量更新所有衛星配置
                updated_configs = await self.get_multiple_satellite_configs(satellite_ids)
