...
from .handover_fault_tolerance_service import (
    HandoverAnomaly, HandoverContext, FallbackAction, 
...
class IntelligentFallbackService:
    ...
    async def make_intelligent_fallback_decision(
        ...
        智能回退決策主函數
        """
        logger.info(f"開始智能回退決策 - 異常類型: {decision_context.anomaly.anomaly_type}")
        ...
        logger.info(f"選擇回退策略: {best_option.strategy.value} - 信心度: {best_option.success_probability:.2f}")
    ...
    async def _create_fallback_option(
        ...
        if strategy == FallbackStrategy.ROLLBACK_TO_SOURCE:
            ...
            return FallbackOption(
                ...
                resource_cost=0.5,
                risk_level=0.4,
                description=f"切換到替代衛星 {best_satellite}",
    ...
    def _calculate_option_score(
        ...
        # 基礎評分組件
        success_score = option.success_probability * self.weight_factors['success_probability']
        
        # 回復時間評分（時間越短越好）
        time_score = max(0, (10.0 - option.estimated_recovery_time) / 10.0) * \
        ...
        # 歷史表現評分
        historical_score = self._get_historical_performance(option.strategy, context) * \
                          self.weight_factors['historical_performance']
        
        total_score = success_score + time_score + cost_score + risk_score + historical_score
    ...
    async def _find_best_alternative_satellite(
        ...
        # 排除當前的源衛星和目標衛星
        candidates = [
            sat for sat in context.available_satellites 
            if sat not in [
    ...
    def _calculate_priority(
        ...
        priority = min(10, base_priority + severity_bonus + success_bonus + urgency_bonus)
        return priority
    ...
    async def record_decision_outcome(
        ...
        strategy_stats['average_recovery_time'] = (
            (current_avg * (total_attempts - 1) + actual_recovery_time) / total_attempts
        )
        
        # 添加到學習緩衝區
        self.learning_buffer.append({
            'context_features': self._extract_context_features(decision_record.context),
        ...
        logger.info(f"記錄決策結果: {decision_id} - {outcome.value} - 成功: {success_achieved}")
    ...
    async def _update_weight_factors(
        ...
        for strategy_samples in success_patterns.values():
            success_recovery_times.extend([s['recovery_time'] for s in strategy_samples])
        
        for strategy_samples in failure_patterns.values():
            failure_recovery_times.extend([s['recovery_time'] for s in strategy_samples])
        ...
        if success_recovery_times and failure_recovery_times:
            ...
            if avg_success_time < avg_failure_time * 0.8:
                self.weight_factors['recovery_time'] = min(0.4, 
                    self.weight_factors['recovery_time'] * 1.1)
                self.weight_factors['success_probability'] = max(0.2,
                    self.weight_factors['success_probability'] * 0.9)
    ...
    async def _update_decision_tree(
        ...
        for anomaly_type in AnomalyType:
            for severity in AnomalySeverity:
                current_strategies = self.decision_tree.get(anomaly_type, {}).get(severity, [])
                ...
                if strategy_scores:
                    ...
                    self.decision_tree[anomaly_type][severity] = [s[0] for s in sorted_strategies]

    def get_performance_metrics(self) -> PerformanceMetrics:
        """獲取性能指標"""
        
        total_decisions = len(self.decision_history)
        successful_decisions = sum(1 for d in self.decision_history if d.success_achieved)
        ...
        average_recovery_time = np.mean([
            d.actual_recovery_time for d in self.decision_history 
        ...
        for i in range(1, len(recent_decisions) + 1):
            ...
            accuracy = sum(1 for d in recent_batch if d.success_achieved) / len(recent_batch)
