...
class EventProcessingConfig:
    ...
    supported_events: List[str] = field(
        default_factory=lambda: ["A4", "D1", "D2", "T1"]
    )
...
class RLDecisionConfig:
    ...
    available_algorithms: List[str] = field(
        default_factory=lambda: ["DQN", "PPO", "SAC"]
    )
...
class MetricsConfig:
    ...
    prometheus_port: int = 9090  # 使用標準Prometheus端口
...
class Settings:
    ...
    def _update_from_env(self):
        ...
        # 指標配置
        self.metrics.enabled = self._get_env_bool(
            "METRICS_ENABLED", self.metrics.enabled
        )
        self.metrics.prometheus_enabled = self._get_env_bool(
            "PROMETHEUS_ENABLED", self.metrics.prometheus_enabled
        )
        self.metrics.prometheus_port = self._get_env_int(
    ...
    def _load_feature_flags(self) -> Dict[str, bool]:
        """載入功能開關"""
        return {
            "use_new_event_processor": self._get_env_bool(
                "USE_NEW_EVENT_PROCESSOR", False
            ),
            "use_new_candidate_selector": self._get_env_bool(
            ...
            "enable_3d_visualization": self._get_env_bool(
                "ENABLE_3D_VISUALIZATION", True
            ),
            "enable_websocket_streaming": self._get_env_bool(
                "ENABLE_WEBSOCKET_STREAMING", True
            ),
            "enable_metrics_collection": self._get_env_bool(
