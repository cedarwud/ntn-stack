...
class OrchestratorMode:
    ...
    LOAD_BALANCING = "load_balancing"  # 添加負載平衡模式
    AB_TESTING = "ab_testing"
    A_B_TESTING = "ab_testing"  # 別名，為了兼容性
    ENSEMBLE = "ensemble"
    ADAPTIVE = "adaptive"  # 添加自適應模式
...
class AlgorithmSelector:
    ...
    async def select_algorithm(
    ...
    ) -> Tuple[Optional[HandoverAlgorithm], Optional[str]]:
        """選擇算法
        ...
        if mode == OrchestratorMode.SINGLE_ALGORITHM:
            ...
            algorithm_name = available_algorithms[
                self._round_robin_counter % len(available_algorithms)
            ]
