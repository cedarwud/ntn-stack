...
class AlgorithmRegistry:
    ...
    async def register_algorithm(
        ...
        try:
            ...
            logger.info(f"算法 '{name}' 註冊成功 (類型: {algorithm_info.algorithm_type.value}, 優先級: {priority})")
    ...
    async def register_algorithm_class(
        ...
        try:
            ...
            raise AlgorithmLoadError(f"Failed to register algorithm class '{name}': {e}")
    ...
    def get_algorithm_by_type(self, algorithm_type: AlgorithmType) -> List[HandoverAlgorithm]:
        """根據類型獲取算法
    ...
    def get_best_algorithm(self, criteria: str = "priority") -> Optional[HandoverAlgorithm]:
        """獲取最佳算法
        ...
        if criteria == "priority":
            # 按優先級選擇
            best_name = max(enabled_algorithms, key=lambda x: self._algorithm_priorities.get(x[0], 0))[0]
            ...
            best_name = min(
                enabled_algorithms,
                key=lambda x: self._algorithm_stats.get(x[0], {}).get('total_execution_time', float('inf'))
            )[0]
            return self._algorithms[best_name]
            ...
            best_name = min(
                enabled_algorithms,
                key=lambda x: self._algorithm_stats.get(x[0], {}).get('error_count', float('inf'))
            )[0]
            return self._algorithms[best_name]
    ...
    async def reload_algorithm(self, name: str) -> bool:
        ...
        try:
            ...
            # 重新註冊
            await self.register_algorithm_class(name, algorithm_class, config, enabled, priority)
    ...
    def get_registry_stats(self) -> Dict[str, Any]:
        ...
        enabled_algorithms = len([name for name, enabled in self._enabled_algorithms.items() if enabled])
    ...
    async def _load_algorithm_from_config(self, name: str, config: Dict[str, Any]) -> None:
        """從配置載入算法"""
        try:
            ...
            await self.register_algorithm_class(name, algorithm_class, algorithm_config, enabled, priority)
