...
try:
    ...
    from scripts.baseline_algorithms.simple_threshold_algorithm import (
...
class BaseTraditionalAdapter(HandoverAlgorithm):
    ...
    def __init__(
        self, name: str, wrapped_algorithm: Any, config: Optional[Dict[str, Any]] = None
    ):
        """初始化適配器
    ...
    def _context_to_observation(self, context: HandoverContext) -> tuple:
        ...
        try:
            ...
            obs_dim = (
                4 + 6 + (6 * max_satellites) + 4
            )  # ue + current_sat + candidates + network
            ...
            if context.current_satellite and context.current_signal_metrics:
                ...
                observation[current_sat_start + 4] = (
                    context.current_signal_metrics.throughput
                )
                observation[current_sat_start + 5] = (
                    context.current_signal_metrics.latency
                )

            # 候選衛星特徵
            candidates_start = current_sat_start + 6
            for i, satellite in enumerate(
                context.candidate_satellites[:max_satellites]
            ):
            ...
            observation[network_start + 3] = network_state.get(
    ...
    def _result_to_decision(
        self, result: "AlgorithmResult", context: HandoverContext
    ) -> HandoverDecision:
        ...
        try:
            ...
            if result.handover_decision == 0:
                ...
                if not target_satellite and context.candidate_satellites:
                    ...
                    best_satellite = max(
                        ...
                        key=lambda s: (
                            s.signal_metrics.sinr if s.signal_metrics else -float("inf")
                        ),
                    )
            ...
            if (
                hasattr(result, "decision_reason")
                and "error" in result.decision_reason.lower()
            ):
                confidence = 0.1
            ...
            decision = HandoverDecision(
                ...
                timing=(
                    datetime.now()
                    if decision_type != HandoverDecisionType.NO_HANDOVER
                    else None
                ),
                decision_reason=getattr(
                    result, "decision_reason", "Algorithm decision"
                ),
...
class SimpleThresholdAlgorithmAdapter(BaseTraditionalAdapter):
    ...
    def __init__(
        self, name: str = "simple_threshold", config: Optional[Dict[str, Any]] = None
    ):
        """初始化簡單閾值算法適配器"""
        ...
        if not BASELINE_ALGORITHMS_AVAILABLE or self.wrapped_algorithm is None:
            ...
            try:
                if context.current_satellite and context.candidate_satellites:
                    current_rsrp = (
                        context.current_satellite.signal_metrics.rsrp
                        if context.current_satellite.signal_metrics
                        else -120
                    )
                    ...
                    for candidate in context.candidate_satellites:
                        candidate_rsrp = (
                            candidate.signal_metrics.rsrp
                            if candidate.signal_metrics
                            else -120
                        )
...
class RandomAlgorithmAdapter(BaseTraditionalAdapter):
    ...
    async def predict_handover(self, context: HandoverContext) -> HandoverDecision:
        ...
        if not BASELINE_ALGORITHMS_AVAILABLE or self.wrapped_algorithm is None:
            ...
            try:
                handover_probability = self.config.get(
                    "handover_probability", 0.3
                )  # 30% 機率換手

                if (
                    context.candidate_satellites
                    and random.random() < handover_probability
                ):
                    # 隨機選擇一個候選衛星
                    ...
                    return HandoverDecision(
                        ...
                        metadata={
                            "fallback": True,
                            "probability": handover_probability,
                        },
                    )
                else:
                    return HandoverDecision(
