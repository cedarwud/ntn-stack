...
import ctypes
from ctypes import Structure, c_char, c_uint32, c_uint16, c_double, c_bool, c_uint64, c_uint8
import structlog
...
class UPFSyncBridge:
    ...
    def load_c_library(self) -> bool:
        ...
        search_paths = [
            ...
            os.path.join(os.path.dirname(__file__), "libsync_algorithm.so"),  # 相對於此檔案
        ]
    ...
    def _define_function_signatures(self):
        ...
        self.lib.sync_algorithm_trigger_handover.argtypes = [
            ctypes.c_char_p, ctypes.c_char_p, c_double
        ]
        self.lib.sync_algorithm_trigger_handover.restype = ctypes.c_int
        
        # sync_algorithm_get_status
        self.lib.sync_algorithm_get_status.argtypes = [ctypes.POINTER(SyncAlgorithmStatus)]
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            self.periodic_update_task = asyncio.create_task(self._periodic_update_loop())
    ...
    async def register_ue(self, ue_info: UEInfo) -> bool:
        ...
        try:
            ...
            if self.lib:
                ...
                ue_context.current_satellite_id = (ue_info.current_satellite_id or "").encode('utf-8')
                ...
                if result != SyncResult.SUCCESS.value:
                    self.logger.error("C 模組 UE 註冊失敗", ue_id=ue_info.ue_id, result=result)
                    return False
        ...
        status = {
            ...
            "ue_management": {
                ...
                "active_handover_count": sum(
                    1 for ue in self.ue_cache.values() 
                    if ue.handover_in_progress
                )
            },
    ...
    async def update_ue_position(self, ue_id: str, latitude: float, longitude: float, altitude: float = 0.0) -> bool:
        """更新 UE 位置"""
        if ue_id not in self.ue_cache:
            return False
        ...
        try:
            ...
            if self.lib:
                ...
                if result != SyncResult.SUCCESS.value:
                    self.logger.warning("C 模組位置更新失敗", ue_id=ue_id, result=result)
    ...
    async def _check_handover_timeouts(self):
        ...
        for ue_info in self.ue_cache.values():
            if (ue_info.handover_in_progress and 
                ue_info.predicted_handover_time and
                current_time - ue_info.predicted_handover_time > timeout_threshold):
    ...
    def _update_average_latency(self, new_latency_ms: float):
        ...
        if total_handovers == 1:
            ...
            self.stats["average_latency_ms"] = (
                (current_avg * (total_handovers - 1) + new_latency_ms) / total_handovers
            )
