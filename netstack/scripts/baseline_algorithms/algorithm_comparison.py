...
try:
    ...
    print("Warning: pandas not available. Some data processing features will be limited.")
...
class ComprehensiveAlgorithmComparison:
    ...
    def __init__(self, output_dir: str = "/home/sat/ntn-stack/results/algorithm_comparison"):
        """
        初始化對比評估系統
    ...
    def evaluate_baseline_algorithms(self):
        ...
        baseline_results = self.baseline_evaluator.compare_algorithms(observations, infos)
    ...
    def evaluate_rl_algorithms(self):
        ...
        for name, model in self.rl_models.items():
            ...
            try:
                ...
                logger.info(f"RL 算法 {name} 評估完成: reward={mean_reward:.3f}±{std_reward:.3f}")
    ...
    def _evaluate_rl_detailed(self, model, model_name: str) -> Dict[str, Any]:
        ...
        metrics['avg_decision_time'] = np.mean(metrics['decision_times']) if metrics['decision_times'] else 0
        metrics['handover_rate'] = metrics['handover_decisions'] / metrics['total_decisions'] if metrics['total_decisions'] > 0 else 0
    ...
    def generate_comparison_report(self):
        ...
        # 保存 JSON 報告
        report_path = self.output_dir / f"algorithm_comparison_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def _generate_overall_comparison(self) -> Dict[str, Any]:
            ...
            if 'individual_results' in baseline_data:
                for algo_name, data in baseline_data['individual_results'].items():
                    ...
                    all_algorithms[algo_name] = {
                        ...
                        'avg_success_rate': metrics.get('average_expected_success_rate', 0),
        ...
        if comparison['performance_ranking']:
            ...
            comparison['key_insights'].append(f"最佳算法: {best_algo['algorithm']} (類型: {best_algo['type']})")
            
            if len(comparison['performance_ranking']) > 1:
                score_gap = best_algo['score'] - comparison['performance_ranking'][1]['score']
...
"""
        ...
        if 'overall_comparison' in report and 'performance_ranking' in report['overall_comparison']:
...
"""
        ...
        if 'overall_comparison' in report and 'key_insights' in report['overall_comparison']:
...
"""
        ...
        # 保存 Markdown 報告
        md_path = self.output_dir / f"algorithm_comparison_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        ...
        if 'baseline' in self.comparison_results:
            ...
            if 'individual_results' in baseline_data:
                for algo_name, data in baseline_data['individual_results'].items():
                    ...
                    success_rates.append(metrics.get('average_expected_success_rate', 0))
    ...
    def _plot_decision_time_distribution(self, output_dir: Path):
        ...
        if decision_times:
            ...
            plt.tight_layout()
            plt.savefig(output_dir / 'decision_time_comparison.png', dpi=300, bbox_inches='tight')
            plt.close()
    ...
    def _plot_success_rate_comparison(self, output_dir: Path):
        ...
        if 'baseline' in self.comparison_results:
            ...
            if 'individual_results' in baseline_data:
                for algo_name, data in baseline_data['individual_results'].items():
                    metrics = data.get('metrics', {})
                    all_success_rates[f"{algo_name} (基準)"] = metrics.get('average_expected_success_rate', 0)
        ...
        if all_success_rates:
            ...
            colors = ['skyblue' if '(基準)' in algo else 'lightcoral' for algo in algorithms]
...
def main():
    ...
    try:
        ...
        # 顯示簡要結果
        if 'overall_comparison' in results and 'performance_ranking' in results['overall_comparison']:
            print("\n性能排名 (前5名):")
            for i, entry in enumerate(results['overall_comparison']['performance_ranking'][:5], 1):
                print(f"{i}. {entry['algorithm']} ({entry['type']}) - 評分: {entry['score']:.3f}")
