...
class UnifiedIntelligentSatelliteFilter:
    ...
    def process_phase1_to_phase2(self, phase1_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        處理階段一到階段二的數據轉換
        ...
        if self.use_unified_system:
            ...
            return self.filter_system.process_complete_filtering(phase1_data, selection_config)
        else:
            return self._process_with_legacy_selector(phase1_data)
    
    def _process_with_legacy_selector(self, phase1_data: Dict[str, Any]) -> Dict[str, Any]:
        """使用現有智能選擇器處理 (後備方案)"""
        ...
        try:
            ...
            # 構建輸出數據
            stage2_output = phase1_data.copy()
            stage2_output["metadata"]["phase2_completion"] = "legacy_intelligent_selector"
            stage2_output["metadata"]["selected_satellites_count"] = len(selected_satellites)
            stage2_output["metadata"]["selection_stats"] = selection_stats
            
            # 更新星座數據，只保留選中的衛星
            for constellation in stage2_output.get("constellations", {}):
                constellation_sats = [sat for sat in selected_satellites 
                                    if sat.get('constellation', '').lower() == constellation.lower()]
                stage2_output["constellations"][constellation]["selected_satellites"] = constellation_sats
...
if __name__ == "__main__":
    ...
    result = filter_system.process_phase1_to_phase2(test_phase1_data)
    
    print("✅ 統一智能篩選系統測試完成")
    print(f"處理結果: {result.get('metadata', {}).get('stage2_completion', 'Unknown')}")