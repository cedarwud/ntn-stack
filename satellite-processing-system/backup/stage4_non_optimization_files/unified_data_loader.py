...
class UnifiedDataLoader:
        ...
        for point in timeseries:
            if point.get("is_visible", False):
                ...
                if elevation > current_window["max_elevation"]:
                    ...
                    # 從物理常數載入默認時間偏移，避免硬編碼數值
                    from shared.constants.physics_constants import SignalConstants
                    signal_consts = SignalConstants()
                    current_window["max_elevation_time"] = point.get("time_offset_seconds", signal_consts.DEFAULT_TIME_OFFSET_SECONDS)
    ...
    def get_visibility_statistics(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        max_elevation = elevation_standards.CRITICAL_ELEVATION_DEG
        satellites_with_visibility = 0
