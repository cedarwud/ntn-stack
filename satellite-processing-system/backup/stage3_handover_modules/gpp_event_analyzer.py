...
class GPPEventAnalyzer:
    ...
    def _assess_measurement_quality(self, rsrp: float, rsrq: float, sinr: float) -> str:
        """Assess overall measurement quality using 3GPP standards."""
        # 🔧 修復：使用3GPP TS 36.133標準閾值
        from shared.constants.physics_constants import SignalConstants
        signal_consts = SignalConstants()

        if rsrp >= signal_consts.RSRP_EXCELLENT and rsrq >= signal_consts.RSRQ_EXCELLENT and sinr >= signal_consts.SINR_EXCELLENT:
            return "EXCELLENT"
        elif rsrp >= signal_consts.RSRP_GOOD and rsrq >= signal_consts.RSRQ_GOOD and sinr >= signal_consts.SINR_GOOD:
            return "GOOD"
        elif rsrp >= signal_consts.RSRP_FAIR and rsrq >= signal_consts.RSRQ_FAIR and sinr >= signal_consts.SINR_FAIR:
            return "FAIR"
        else:
            return "POOR"
