...
class DynamicThresholdController:
    ...
    def _analyze_signal_conditions(self, satellites: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # Assess quality levels - 使用3GPP標準閾值
        from shared.constants.physics_constants import SignalConstants
        signal_consts = SignalConstants()

        if avg_rsrp >= signal_consts.RSRP_GOOD and avg_rsrq >= signal_consts.RSRQ_EXCELLENT and avg_sinr >= signal_consts.SINR_GOOD:
            quality = 'EXCELLENT'
        elif avg_rsrp >= signal_consts.RSRP_FAIR and avg_rsrq >= signal_consts.RSRQ_GOOD and avg_sinr >= signal_consts.SINR_FAIR:
            quality = 'GOOD'
        elif avg_rsrp >= signal_consts.RSRP_POOR and avg_rsrq >= signal_consts.RSRQ_FAIR and avg_sinr >= signal_consts.SINR_POOR:
            quality = 'FAIR'
        else:
            quality = 'POOR'
