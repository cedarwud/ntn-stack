...
class ValidationSnapshotBase(ABC):
    ...
    def __init__(self, stage_number: int, stage_name: str, snapshot_dir: str = "data/validation_snapshots"):
        """
        初始化驗證快照基礎
        ...
        # 智能選擇數據目錄
        if snapshot_dir.startswith("/satellite-processing/") and not os.path.exists("/satellite-processing"):
            # 容器外執行時，使用本地目錄
    ...
    def get_pipeline_validation_summary() -> Dict[str, Any]:
        ...
        """
        # 智能選擇快照目錄
        if not os.path.exists("/satellite-processing"):
            # 容器外執行時，使用本地目錄
