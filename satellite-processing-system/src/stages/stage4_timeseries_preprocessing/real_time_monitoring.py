...
class RealTimeMonitoringEngine:
    ...
    def _predict_coverage_changes(self, satellites_data: List[Dict], horizon_minutes: int) -> Dict[str, Any]:
        ...
        prediction = {
            ...
            'predicted_coverage_1min': max(0.0, min(1.0, current_coverage + np.random.normal(0, 0.02))),
            'predicted_coverage_5min': max(0.0, min(1.0, current_coverage + np.random.normal(0, 0.05))),
            'predicted_coverage_10min': max(0.0, min(1.0, current_coverage + np.random.normal(0, 0.08))),
