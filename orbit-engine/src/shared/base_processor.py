...
class BaseStageProcessor(ABC):
    ...
    def cleanup_previous_output(self) -> None:
        ...
        # 1. 🗑️ 安全清理階段輸出資料夾內容 (Docker Volume安全)
            ...
            try:
                ...
                for item in self.output_dir.iterdir():
                    if item.is_file():
                        ...
                        shutil.rmtree(item)
                        cleaned_items += 1
                cleanup_details.append(f"cleaned_output_dir_contents: {self.output_dir}")
                self.logger.info(f"🗑️ 已清理階段資料夾內容: {self.output_dir}")
            except OSError as e:
                if e.errno == 16:  # Device or resource busy
                    self.logger.warning(f"⚠️ Docker Volume檢測 - 跳過目錄清理: {self.output_dir}")
                    cleanup_details.append(f"docker_volume_skipped: {self.output_dir}")
                else:
                    raise
