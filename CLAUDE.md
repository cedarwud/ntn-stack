...
- **絕對禁止**: 使用當前系統時間作為軌道計算的基準時間
- **原因**: 當前時間與TLE數據時間差可能導致軌道預測完全不準確
...
### ⚠️ 關鍵問題案例
**問題**: 8000+顆衛星計算結果顯示0顆可見
**原因**: 使用當前時間(2025-09-08)計算，但TLE數據是(2025-09-02/03)，5-6天時間差導致軌道預測完全偏離
**解決**: 修改 `orbital_calculation_processor.py` 使用TLE epoch時間作為計算基準

### 📝 檢查清單
**任何軌道計算前必須確認**：
- [ ] 使用的是TLE數據的epoch時間，不是當前時間
- [ ] 時間差超過3天時有警告提示
- [ ] 計算基準時間已正確記錄在metadata中
...
- **[學術級數據使用標準](./docs/academic_data_standards.md)** 🎓 **強制遵循！**
  - **何時查看**: 任何涉及數據處理、算法實現、模型建立時
  - **包含內容**: Grade A/B/C分級標準、禁止項目清單、檢查清單、參考文獻
  - **適用場景**: 所有六階段處理器開發、論文撰寫、代碼審查
...
**軌道動力學**:
- TLE數據: Space-Track.org實時數據
- 算法: 完整SGP4/SDP4實現
- 時間: GPS/UTC標準時間

🚨 **軌道計算時間基準強制原則** (零容忍):
- **✅ 必須使用**: TLE數據的epoch時間作為SGP4計算基準
- **❌ 絕對禁止**: 使用當前系統時間 (`datetime.now()`) 進行軌道計算
- **實例教訓**: 8000+顆衛星→0顆可見 = 使用錯誤時間基準
- **風險警告**: 時間差>3天軌道預測嚴重失準
