...
loader_spec = importlib.util.spec_from_file_location(
    "local_tle_loader", "netstack/src/services/satellite/local_tle_loader.py"
)
...
def test_coordinate_engine():
    ...
    # 2. 載入測試衛星數據
    print("\n📡 2. 載入 TLE 數據")
    loader = LocalTLELoader("netstack/tle_data")
    ...
    filtered_satellites = engine.filter_visible_satellites(
        test_satellites, reference_time
    )
    ...
    print(
        f"  - 篩選效率: {(len(test_satellites) - len(filtered_satellites))/len(test_satellites)*100:.1f}% 減少"
    )
    ...
    optimal_window = engine.find_optimal_timewindow(
        filtered_satellites, window_hours=6, reference_time=reference_time
    )
    ...
    display_data = engine.generate_display_optimized_data(
        optimal_window, acceleration=60, distance_scale=0.1
    )
