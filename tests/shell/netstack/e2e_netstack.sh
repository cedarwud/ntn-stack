...
main() {
    ...
    test_health_check
    health_status=$?
    if [ $health_status -eq 0 ]; then passed=$((passed + 1)); else failed=$((failed + 1)); overall_status=1; fi
    echo ""
    
    test_list_ues
    list_ues_status=$?
    if [ $list_ues_status -eq 0 ]; then passed=$((passed + 1)); else failed=$((failed + 1)); overall_status=1; fi
    echo ""
    
    test_slice_types
    slice_types_status=$?
    if [ $slice_types_status -eq 0 ]; then passed=$((passed + 1)); else failed=$((failed + 1)); overall_status=1; fi
    echo ""
    ...
    if [ $ue_exists -eq 0 ]; then
        ...
        test_ue_stats
        stats_status=$?
        if [ $stats_status -eq 0 ]; then passed=$((passed + 1)); else failed=$((failed + 1)); overall_status=1; fi
        echo ""
        
        test_slice_switch "uRLLC"
        switch_urllc_status=$?
        if [ $switch_urllc_status -eq 0 ]; then passed=$((passed + 1)); else failed=$((failed + 1)); overall_status=1; fi
        echo ""
        
        test_slice_switch "eMBB"
        switch_embb_status=$?
        if [ $switch_embb_status -eq 0 ]; then passed=$((passed + 1)); else failed=$((failed + 1)); overall_status=1; fi
        echo ""
        ...
        stress_test
        stress_status=$?
        if [ $stress_status -eq 0 ]; then passed=$((passed + 1)); else failed=$((failed + 1)); overall_status=1; fi
    else
