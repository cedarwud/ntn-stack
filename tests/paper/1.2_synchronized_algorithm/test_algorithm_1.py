...
async def test_algorithm_1_core():
    ...
    try:
        ...
        access_info = AccessInfo(
            ue_id="ue_001", satellite_id="sat_001", access_quality=0.85
        )
        ...
        print(
            f"   真實性檢查: {'✅ 合理' if realistic_result else '❌ 疑似測試模式'} (>5ms)"
        )
        print(
            f"   預測時間: {datetime.fromtimestamp(handover_time).strftime('%H:%M:%S')}"
        )
        ...
        status_valid = (
            ...
            and "last_update_time" in status
            and "active_ue_count" in status
        ...
        print(
            f"✅ 預測準確性: {successful_predictions}/{prediction_count} ({prediction_accuracy:.1%})"
        )
        ...
        if handover_latency_measurements:
            avg_latency = sum(handover_latency_measurements) / len(
                handover_latency_measurements
            )
            ...
            algorithm_reasonable = (
                100.0 <= avg_latency <= 10000.0
            )  # 算法計算時間：100ms-10s
            print(
                f"   算法計算時間評估: {'✅ 合理範圍' if algorithm_reasonable else '❌ 異常'}（100ms-10s)"
            )
...
async def test_integration_bridge():
    ...
    try:
        ...
        config = BridgeConfiguration(
            integration_mode=IntegrationMode.PAPER_ONLY, enhanced_features_enabled=False
        )
        ...
        print(f"✅ 模式換手: {switch_result['success']}")
        test_results.append(("模式換手", switch_result["success"]))
...
async def main():
    ...
    print(
        f"   ✅ 真實計算: {'正常' if any(name == '真實二分搜尋' and result for name, result in all_results) else '異常'}"
    )
    print(
        f"   ✅ 延遲測量: {'合理' if any(name == '延遲合理性' and result for name, result in all_results) else '異常'}"
    )
    print(
        f"   ✅ 週期性更新: {'正常' if any(name == '週期性更新' and result for name, result in all_results) else '異常'}"
    )
