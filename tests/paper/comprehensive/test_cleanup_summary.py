...
python paper/comprehensive/test_cleanup_summary.py
"""
...
def generate_cleanup_report():
    ...
    test_mapping = {
        ...
        "1.2 同步演算法 (Algorithm 1)": ["test_algorithm_1.py"],
        "1.3 快速衛星預測演算法 (Algorithm 2)": ["test_algorithm_2.py"],
        "綜合測試執行器": [
    ...
    for i, (script, description) in enumerate(usage_order, 1):
        print(f"   {i}. python paper/comprehensive/{script}")
        print(f"      {description}")
        if i <= 3:  # 只在前3個顯示路徑
            print()
